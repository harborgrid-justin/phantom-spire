...
interface ResourceUtilizationDashboardData {
  id: string;
  title: string;
  status: string;
  createdAt: string;
...
export default function ResourceUtilizationDashboardPage() {
  const [data, setData] = useState<ResourceUtilizationDashboardData[]>([]);
  ...
  const {
    ...
    addNotification,
    removeNotification,
  } = useServicePage('case-management-resource-utilization-dashboard');
  ...
  const fetchData = async () => {
    try {
      ...
      const mockData: ResourceUtilizationDashboardData[] = [
        {
          id: '1',
      ...
      addNotification({
        type: 'success',
        message: 'Resource Utilization Dashboard data loaded successfully',
        duration: 3000
      });
    } catch (error) {
      console.error('Error fetching resource-utilization-dashboard data:', error);
      addNotification({
        type: 'error',
        message: 'Failed to load Resource Utilization Dashboard data',
        duration: 5000
      });
    } finally {
      setLoading(false);
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">Resource usage optimization analytics</p>
            </div>
          ...
          {data.length === 0 && (
            <div className="text-center py-12">
              <div className="text-gray-500">
                ...
                <p className="text-sm">Get started by creating your first resource utilization dashboard entry.</p>
              </div>
            </div>
          )}
        </div>
