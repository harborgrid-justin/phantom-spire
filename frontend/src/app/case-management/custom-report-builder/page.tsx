...
interface CustomReportBuilderData {
  id: string;
  title: string;
  status: string;
  createdAt: string;
...
export default function CustomReportBuilderPage() {
  const [data, setData] = useState<CustomReportBuilderData[]>([]);
  ...
  const {
    ...
    addNotification,
    removeNotification,
  } = useServicePage('case-management-custom-report-builder');
  ...
  const fetchData = async () => {
    try {
      ...
      const mockData: CustomReportBuilderData[] = [
        {
          id: '1',
      ...
      addNotification({
        type: 'success',
        message: 'Custom Report Builder data loaded successfully',
        duration: 3000
      });
    } catch (error) {
      console.error('Error fetching custom-report-builder data:', error);
      addNotification({
        type: 'error',
        message: 'Failed to load Custom Report Builder data',
        duration: 5000
      });
    } finally {
      setLoading(false);
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">Drag-and-drop custom report creation</p>
            </div>
          ...
          {data.length === 0 && (
            <div className="text-center py-12">
              <div className="text-gray-500">
                ...
                <p className="text-sm">Get started by creating your first custom report builder entry.</p>
              </div>
            </div>
          )}
        </div>
