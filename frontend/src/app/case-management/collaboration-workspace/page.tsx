...
interface CollaborationWorkspaceData {
  id: string;
  title: string;
  status: string;
  createdAt: string;
...
export default function CollaborationWorkspacePage() {
  const [data, setData] = useState<CollaborationWorkspaceData[]>([]);
  ...
  const {
    ...
    addNotification,
    removeNotification,
  } = useServicePage('case-management-collaboration-workspace');
  ...
  const fetchData = async () => {
    try {
      ...
      const mockData: CollaborationWorkspaceData[] = [
        {
          id: '1',
      ...
      addNotification({
        type: 'success',
        message: 'Collaboration Workspace data loaded successfully',
        duration: 3000
      });
    } catch (error) {
      console.error('Error fetching collaboration-workspace data:', error);
      addNotification({
        type: 'error',
        message: 'Failed to load Collaboration Workspace data',
        duration: 5000
      });
    } finally {
      setLoading(false);
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">Team collaboration tools for investigation teams</p>
            </div>
          ...
          {data.length === 0 && (
            <div className="text-center py-12">
              <div className="text-gray-500">
                ...
                <p className="text-sm">Get started by creating your first collaboration workspace entry.</p>
              </div>
            </div>
          )}
        </div>
