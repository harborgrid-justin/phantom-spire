...
interface UserDelegationSystemData {
  id: string;
  name: string;
...
export default function UserDelegationSystemPage() {
  const [data, setData] = useState<UserDelegationSystemData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-delegation-system');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Role delegation and temporary access management</p>
        </div>
        <button
