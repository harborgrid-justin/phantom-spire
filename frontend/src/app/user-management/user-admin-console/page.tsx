...
interface UserAdminConsoleData {
  id: string;
  name: string;
...
export default function UserAdminConsolePage() {
  const [data, setData] = useState<UserAdminConsoleData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-admin-console');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            🎛️ User Admin Console
          </h1>
          <p className="text-gray-600">Advanced administrative console for user management</p>
        </div>
        <button
