...
interface PermissionMatrixManagerData {
  id: string;
  name: string;
...
export default function PermissionMatrixManagerPage() {
  const [data, setData] = useState<PermissionMatrixManagerData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/permission-matrix-manager');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Comprehensive permission matrix and access control</p>
        </div>
        <button
