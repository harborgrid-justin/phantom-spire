...
interface UserWellnessDashboardData {
  id: string;
  name: string;
...
export default function UserWellnessDashboardPage() {
  const [data, setData] = useState<UserWellnessDashboardData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-wellness-dashboard');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Employee wellness tracking and work-life balance</p>
        </div>
        <button
