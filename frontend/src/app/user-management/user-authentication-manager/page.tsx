...
interface UserAuthenticationManagerData {
  id: string;
  name: string;
...
export default function UserAuthenticationManagerPage() {
  const [data, setData] = useState<UserAuthenticationManagerData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-authentication-manager');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Multi-factor authentication and security methods</p>
        </div>
        <button
