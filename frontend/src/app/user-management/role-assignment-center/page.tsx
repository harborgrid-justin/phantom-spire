...
interface RoleAssignmentCenterData {
  id: string;
  name: string;
...
export default function RoleAssignmentCenterPage() {
  const [data, setData] = useState<RoleAssignmentCenterData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/role-assignment-center');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Role assignment workflows and approval processes</p>
        </div>
        <button
