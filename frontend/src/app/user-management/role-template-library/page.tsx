...
interface RoleTemplateLibraryData {
  id: string;
  name: string;
...
export default function RoleTemplateLibraryPage() {
  const [data, setData] = useState<RoleTemplateLibraryData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/role-template-library');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Pre-defined role templates and quick deployment</p>
        </div>
        <button
