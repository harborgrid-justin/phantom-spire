...
interface UserPrivacyControlCenterData {
  id: string;
  name: string;
...
export default function UserPrivacyControlCenterPage() {
  const [data, setData] = useState<UserPrivacyControlCenterData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-privacy-control-center');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            🛡️ User Privacy Control Center
          </h1>
          <p className="text-gray-600">Personal data privacy and GDPR compliance management</p>
        </div>
        <button
