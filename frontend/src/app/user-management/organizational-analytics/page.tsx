...
interface OrganizationalAnalyticsData {
  id: string;
  name: string;
...
export default function OrganizationalAnalyticsPage() {
  const [data, setData] = useState<OrganizationalAnalyticsData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/organizational-analytics');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Organizational structure analytics and insights</p>
        </div>
        <button
