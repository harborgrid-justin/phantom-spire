...
interface UserRecognitionSystemData {
  id: string;
  name: string;
...
export default function UserRecognitionSystemPage() {
  const [data, setData] = useState<UserRecognitionSystemData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-recognition-system');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Employee recognition and achievement tracking</p>
        </div>
        <button
