...
interface UserDirectoryPortalData {
  id: string;
  name: string;
...
export default function UserDirectoryPortalPage() {
  const [data, setData] = useState<UserDirectoryPortalData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-directory-portal');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Comprehensive user directory and profile management</p>
        </div>
        <button
