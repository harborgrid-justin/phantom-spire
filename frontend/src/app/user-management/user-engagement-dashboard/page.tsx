...
interface UserEngagementDashboardData {
  id: string;
  name: string;
...
export default function UserEngagementDashboardPage() {
  const [data, setData] = useState<UserEngagementDashboardData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-engagement-dashboard');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">User activity monitoring and engagement metrics</p>
        </div>
        <button
