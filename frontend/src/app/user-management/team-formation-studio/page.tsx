...
interface TeamFormationStudioData {
  id: string;
  name: string;
...
export default function TeamFormationStudioPage() {
  const [data, setData] = useState<TeamFormationStudioData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/team-formation-studio');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Dynamic team creation and collaboration setup</p>
        </div>
        <button
