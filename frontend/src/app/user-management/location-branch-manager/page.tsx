...
interface LocationBranchManagerData {
  id: string;
  name: string;
...
export default function LocationBranchManagerPage() {
  const [data, setData] = useState<LocationBranchManagerData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/location-branch-manager');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Multi-location and branch office management</p>
        </div>
        <button
