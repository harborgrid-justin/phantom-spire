...
interface PrivilegeEscalationControlData {
  id: string;
  name: string;
...
export default function PrivilegeEscalationControlPage() {
  const [data, setData] = useState<PrivilegeEscalationControlData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/privilege-escalation-control');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Temporary privilege escalation and time-based access</p>
        </div>
        <button
