...
interface UserAccountManagerData {
  id: string;
  name: string;
...
export default function UserAccountManagerPage() {
  const [data, setData] = useState<UserAccountManagerData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-account-manager');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Account status, security, and access management</p>
        </div>
        <button
