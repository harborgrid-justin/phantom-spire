...
interface UserSynchronizationHubData {
  id: string;
  name: string;
...
export default function UserSynchronizationHubPage() {
  const [data, setData] = useState<UserSynchronizationHubData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-synchronization-hub');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            🔄 User Synchronization Hub
          </h1>
          <p className="text-gray-600">External directory synchronization and integration</p>
        </div>
        <button
