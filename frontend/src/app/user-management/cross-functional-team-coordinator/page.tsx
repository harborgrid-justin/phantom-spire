...
interface CrossFunctionalTeamCoordinatorData {
  id: string;
  name: string;
...
export default function CrossFunctionalTeamCoordinatorPage() {
  const [data, setData] = useState<CrossFunctionalTeamCoordinatorData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/cross-functional-team-coordinator');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Cross-departmental team coordination and management</p>
        </div>
        <button
