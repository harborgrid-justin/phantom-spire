...
interface RoleHierarchyBuilderData {
  id: string;
  name: string;
...
export default function RoleHierarchyBuilderPage() {
  const [data, setData] = useState<RoleHierarchyBuilderData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/role-hierarchy-builder');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            🏗️ Role Hierarchy Builder
          </h1>
          <p className="text-gray-600">Hierarchical role structure and inheritance management</p>
        </div>
        <button
