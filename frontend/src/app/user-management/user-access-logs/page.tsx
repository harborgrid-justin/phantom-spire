...
export default function UserAccessLogsPage() {
  const [data, setData] = useState<UserAccessLogsData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-access-logs');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Detailed access logging and audit trail</p>
        </div>
        <button
