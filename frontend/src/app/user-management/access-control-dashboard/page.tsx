...
interface AccessControlDashboardData {
  id: string;
  name: string;
...
export default function AccessControlDashboardPage() {
  const [data, setData] = useState<AccessControlDashboardData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/access-control-dashboard');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            🛡️ Access Control Dashboard
          </h1>
          <p className="text-gray-600">Real-time access control monitoring and management</p>
        </div>
        <button
