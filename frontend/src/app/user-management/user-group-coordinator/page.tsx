...
interface UserGroupCoordinatorData {
  id: string;
  name: string;
...
export default function UserGroupCoordinatorPage() {
  const [data, setData] = useState<UserGroupCoordinatorData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-group-coordinator');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Dynamic user group management and coordination</p>
        </div>
        <button
