...
interface UserSecurityDashboardData {
  id: string;
  name: string;
...
export default function UserSecurityDashboardPage() {
  const [data, setData] = useState<UserSecurityDashboardData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-security-dashboard');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Personal security status and threat monitoring</p>
        </div>
        <button
