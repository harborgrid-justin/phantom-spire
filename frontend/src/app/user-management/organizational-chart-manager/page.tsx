...
interface OrganizationalChartManagerData {
  id: string;
  name: string;
...
export default function OrganizationalChartManagerPage() {
  const [data, setData] = useState<OrganizationalChartManagerData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/organizational-chart-manager');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Interactive organizational charts and reporting structures</p>
        </div>
        <button
