...
interface RoleAnalyticsInsightsData {
  id: string;
  name: string;
...
export default function RoleAnalyticsInsightsPage() {
  const [data, setData] = useState<RoleAnalyticsInsightsData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/role-analytics-insights');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Role usage analytics and optimization insights</p>
        </div>
        <button
