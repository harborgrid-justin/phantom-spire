...
interface DepartmentManagementHubData {
  id: string;
  name: string;
...
export default function DepartmentManagementHubPage() {
  const [data, setData] = useState<DepartmentManagementHubData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/department-management-hub');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            🏛️ Department Management Hub
          </h1>
          <p className="text-gray-600">Department creation, management, and restructuring</p>
        </div>
        <button
