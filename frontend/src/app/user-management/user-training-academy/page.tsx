...
interface UserTrainingAcademyData {
  id: string;
  name: string;
...
export default function UserTrainingAcademyPage() {
  const [data, setData] = useState<UserTrainingAcademyData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-training-academy');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            🎓 User Training Academy
          </h1>
          <p className="text-gray-600">Comprehensive training programs and skill development</p>
        </div>
        <button
