...
interface UserSupportPortalData {
  id: string;
  name: string;
...
export default function UserSupportPortalPage() {
  const [data, setData] = useState<UserSupportPortalData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-support-portal');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Self-service support and helpdesk integration</p>
        </div>
        <button
