...
interface UserBulkOperationsData {
  id: string;
  name: string;
...
export default function UserBulkOperationsPage() {
  const [data, setData] = useState<UserBulkOperationsData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-bulk-operations');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Bulk user operations and mass management tools</p>
        </div>
        <button
