...
interface ReportingStructureDesignerData {
  id: string;
  name: string;
...
export default function ReportingStructureDesignerPage() {
  const [data, setData] = useState<ReportingStructureDesignerData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/reporting-structure-designer');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Hierarchical reporting relationships and chains of command</p>
        </div>
        <button
