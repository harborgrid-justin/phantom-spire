...
interface RoleDefinitionStudioData {
  id: string;
  name: string;
...
export default function RoleDefinitionStudioPage() {
  const [data, setData] = useState<RoleDefinitionStudioData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/role-definition-studio');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            🎨 Role Definition Studio
          </h1>
          <p className="text-gray-600">Visual role creation and permission assignment</p>
        </div>
        <button
