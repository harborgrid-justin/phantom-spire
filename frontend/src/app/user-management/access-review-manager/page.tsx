...
interface AccessReviewManagerData {
  id: string;
  name: string;
...
export default function AccessReviewManagerPage() {
  const [data, setData] = useState<AccessReviewManagerData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/access-review-manager');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Periodic access reviews and certification workflows</p>
        </div>
        <button
