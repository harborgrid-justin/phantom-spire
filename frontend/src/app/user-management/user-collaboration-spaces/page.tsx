...
interface UserCollaborationSpacesData {
  id: string;
  name: string;
...
export default function UserCollaborationSpacesPage() {
  const [data, setData] = useState<UserCollaborationSpacesData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-collaboration-spaces');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Digital collaboration environments and workspace management</p>
        </div>
        <button
