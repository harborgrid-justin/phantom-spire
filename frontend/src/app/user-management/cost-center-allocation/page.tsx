...
interface CostCenterAllocationData {
  id: string;
  name: string;
...
export default function CostCenterAllocationPage() {
  const [data, setData] = useState<CostCenterAllocationData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/cost-center-allocation');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Cost center assignment and budget allocation</p>
        </div>
        <button
