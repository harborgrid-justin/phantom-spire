...
interface UserDeviceManagementData {
  id: string;
  name: string;
...
export default function UserDeviceManagementPage() {
  const [data, setData] = useState<UserDeviceManagementData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-device-management');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Personal device registration and security management</p>
        </div>
        <button
