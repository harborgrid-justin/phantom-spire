...
interface UserProductivityInsightsData {
  id: string;
  name: string;
...
export default function UserProductivityInsightsPage() {
  const [data, setData] = useState<UserProductivityInsightsData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-productivity-insights');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Productivity analytics and performance insights</p>
        </div>
        <button
