...
interface PermissionAuditSystemData {
  id: string;
  name: string;
...
export default function PermissionAuditSystemPage() {
  const [data, setData] = useState<PermissionAuditSystemData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/permission-audit-system');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Permission usage auditing and compliance tracking</p>
        </div>
        <button
