...
interface UserFeedbackCenterData {
  id: string;
  name: string;
...
export default function UserFeedbackCenterPage() {
  const [data, setData] = useState<UserFeedbackCenterData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-feedback-center');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">User feedback collection and sentiment analysis</p>
        </div>
        <button
