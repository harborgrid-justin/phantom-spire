...
interface UserDataGovernanceData {
  id: string;
  name: string;
...
export default function UserDataGovernancePage() {
  const [data, setData] = useState<UserDataGovernanceData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-data-governance');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Personal data governance and retention policies</p>
        </div>
        <button
