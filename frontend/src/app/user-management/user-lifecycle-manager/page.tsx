...
interface UserLifecycleManagerData {
  id: string;
  name: string;
...
export default function UserLifecycleManagerPage() {
  const [data, setData] = useState<UserLifecycleManagerData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-lifecycle-manager');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Complete user lifecycle from onboarding to offboarding</p>
        </div>
        <button
