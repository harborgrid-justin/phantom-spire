...
interface UserProfileDashboardData {
  id: string;
  name: string;
...
export default function UserProfileDashboardPage() {
  const [data, setData] = useState<UserProfileDashboardData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-profile-dashboard');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Centralized user profile management and customization</p>
        </div>
        <button
