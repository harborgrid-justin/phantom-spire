...
interface UserCommunicationHubData {
  id: string;
  name: string;
...
export default function UserCommunicationHubPage() {
  const [data, setData] = useState<UserCommunicationHubData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-communication-hub');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Internal communication and announcement system</p>
        </div>
        <button
