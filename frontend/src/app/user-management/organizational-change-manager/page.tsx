...
interface OrganizationalChangeManagerData {
  id: string;
  name: string;
...
export default function OrganizationalChangeManagerPage() {
  const [data, setData] = useState<OrganizationalChangeManagerData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/organizational-change-manager');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Change management for organizational restructuring</p>
        </div>
        <button
