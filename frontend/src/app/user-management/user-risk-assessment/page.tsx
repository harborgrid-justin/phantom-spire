...
interface UserRiskAssessmentData {
  id: string;
  name: string;
...
export default function UserRiskAssessmentPage() {
  const [data, setData] = useState<UserRiskAssessmentData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-risk-assessment');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            ⚠️ User Risk Assessment
          </h1>
          <p className="text-gray-600">User-based risk assessment and mitigation</p>
        </div>
        <button
