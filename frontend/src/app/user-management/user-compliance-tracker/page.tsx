...
interface UserComplianceTrackerData {
  id: string;
  name: string;
...
export default function UserComplianceTrackerPage() {
  const [data, setData] = useState<UserComplianceTrackerData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-compliance-tracker');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            ✅ User Compliance Tracker
          </h1>
          <p className="text-gray-600">Individual compliance status and certification tracking</p>
        </div>
        <button
