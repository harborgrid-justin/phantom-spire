...
interface UserSettingsCenterData {
  id: string;
  name: string;
...
export default function UserSettingsCenterPage() {
  const [data, setData] = useState<UserSettingsCenterData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-settings-center');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            ⚙️ User Settings Center
          </h1>
          <p className="text-gray-600">Personal settings and preferences management</p>
        </div>
        <button
