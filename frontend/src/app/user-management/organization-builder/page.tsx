...
interface OrganizationBuilderData {
  id: string;
  name: string;
...
export default function OrganizationBuilderPage() {
  const [data, setData] = useState<OrganizationBuilderData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/organization-builder');
      if (!response.ok) {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600">Visual organization structure design and management</p>
        </div>
        <button
