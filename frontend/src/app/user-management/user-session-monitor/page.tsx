...
interface UserSessionMonitorData {
  id: string;
  name: string;
...
export default function UserSessionMonitorPage() {
  const [data, setData] = useState<UserSessionMonitorData[]>([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/user-management/user-session-monitor');
      if (!response.ok) {
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            👁️ User Session Monitor
          </h1>
          <p className="text-gray-600">Session management and concurrent access control</p>
        </div>
        <button
