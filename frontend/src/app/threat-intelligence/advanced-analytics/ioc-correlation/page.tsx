...
export default function IocCorrelationPage() {
  const { 
    data, 
    loading, 
    error, 
    ...
    notifications,
    removeNotification 
  } = useServicePage('threat-intel-advanced-analytics-ioc-correlation');
  ...
  const loadData = async () => {
    try {
      ...
      await execute('get-data', { 
        endpoint: '/api/v1/threat-intelligence/advanced-analytics/ioc-correlation',
        filter,
        search: searchTerm 
      });
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      await execute('export-data', { 
        endpoint: '/api/v1/threat-intelligence/advanced-analytics/ioc-correlation/export',
        format: 'json'
      });
    } catch (err) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-center h-64">
          <div className="text-lg">Loading IOC Correlation Engine...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        <div className="flex items-center space-x-4 text-sm">
          <div className={`flex items-center ${connected ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            {connected ? 'IOC Correlation Engine System Online' : 'IOC Correlation Engine System Offline'}
          </div>
          {stats && (
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">🔗 IOC Correlation Engine</h1>
          <p className="text-gray-600 mt-2">Advanced correlation analysis for indicators of compromise</p>
        </div>
