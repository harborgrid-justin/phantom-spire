...
export default function CampaignAnalysisPage() {
  const { 
    ...
    notifications,
    removeNotification 
  } = useServicePage('threat-intel-advanced-analytics-campaign-analysis');
  ...
  const loadData = async () => {
    try {
      ...
      await execute('get-data', { 
        endpoint: '/api/v1/threat-intelligence/advanced-analytics/campaign-analysis',
        filter,
        search: searchTerm 
      });
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      await execute('export-data', { 
        endpoint: '/api/v1/threat-intelligence/advanced-analytics/campaign-analysis/export',
        format: 'json'
      });
    } catch (err) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-center h-64">
          <div className="text-lg">Loading Campaign Analysis...</div>
        </div>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        <div className="flex items-center space-x-4 text-sm">
          <div className={`flex items-center ${connected ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            {connected ? 'Campaign Analysis System Online' : 'Campaign Analysis System Offline'}
          </div>
          {stats && (
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-2">Deep analysis of threat campaigns and attack patterns</p>
        </div>
