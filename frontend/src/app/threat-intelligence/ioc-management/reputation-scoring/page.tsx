...
export default function ReputationScoringPage() {
  const { 
    data, 
    loading, 
    error, 
    ...
    notifications,
    removeNotification 
  } = useServicePage('threat-intel-ioc-management-reputation-scoring');
  ...
  const loadData = async () => {
    try {
      ...
      await execute('get-data', { 
        endpoint: '/api/v1/threat-intelligence/ioc-management/reputation-scoring',
        filter,
        search: searchTerm 
      });
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      await execute('export-data', { 
        endpoint: '/api/v1/threat-intelligence/ioc-management/reputation-scoring/export',
        format: 'json'
      });
    } catch (err) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-center h-64">
          <div className="text-lg">Loading IOC Reputation Scoring...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        <div className="flex items-center space-x-4 text-sm">
          <div className={`flex items-center ${connected ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            {connected ? 'IOC Reputation Scoring System Online' : 'IOC Reputation Scoring System Offline'}
          </div>
          {stats && (
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">⭐ IOC Reputation Scoring</h1>
          <p className="text-gray-600 mt-2">Reputation scoring and confidence rating for IOCs</p>
        </div>
