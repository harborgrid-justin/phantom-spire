...
export default function ExportImportHubPage() {
  const { 
    data, 
    loading, 
    error, 
    ...
    notifications,
    removeNotification 
  } = useServicePage('threat-intel-ioc-management-export-import-hub');
  ...
  const loadData = async () => {
    try {
      ...
      await execute('get-data', { 
        endpoint: '/api/v1/threat-intelligence/ioc-management/export-import-hub',
        filter,
        search: searchTerm 
      });
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      await execute('export-data', { 
        endpoint: '/api/v1/threat-intelligence/ioc-management/export-import-hub/export',
        format: 'json'
      });
    } catch (err) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-center h-64">
          <div className="text-lg">Loading IOC Export/Import Hub...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        <div className="flex items-center space-x-4 text-sm">
          <div className={`flex items-center ${connected ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            {connected ? 'IOC Export/Import Hub System Online' : 'IOC Export/Import Hub System Offline'}
          </div>
          {stats && (
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">🔄 IOC Export/Import Hub</h1>
          <p className="text-gray-600 mt-2">Centralized hub for IOC data export and import</p>
        </div>
