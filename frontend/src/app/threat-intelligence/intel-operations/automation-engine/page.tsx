...
export default function AutomationEnginePage() {
  const { 
    ...
    notifications,
    removeNotification 
  } = useServicePage('threat-intel-intel-operations-automation-engine');
  ...
  const loadData = async () => {
    try {
      ...
      await execute('get-data', { 
        endpoint: '/api/v1/threat-intelligence/intel-operations/automation-engine',
        filter,
        search: searchTerm 
      });
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      await execute('export-data', { 
        endpoint: '/api/v1/threat-intelligence/intel-operations/automation-engine/export',
        format: 'json'
      });
    } catch (err) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-center h-64">
          <div className="text-lg">Loading Threat Intelligence Automation...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        <div className="flex items-center space-x-4 text-sm">
          <div className={`flex items-center ${connected ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            {connected ? 'Threat Intelligence Automation System Online' : 'Threat Intelligence Automation System Offline'}
          </div>
          {stats && (
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">🤖 Threat Intelligence Automation</h1>
          <p className="text-gray-600 mt-2">Automated threat intelligence processing and analysis</p>
        </div>
