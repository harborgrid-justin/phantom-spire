...
export default function ApiManagementPage() {
  const { 
    ...
    notifications,
    removeNotification 
  } = useServicePage('threat-intel-intel-operations-api-management');
  ...
  const loadData = async () => {
    try {
      ...
      await execute('get-data', { 
        endpoint: '/api/v1/threat-intelligence/intel-operations/api-management',
        filter,
        search: searchTerm 
      });
    } catch (err) {
  ...
  const handleExport = async () => {
    try {
      await execute('export-data', { 
        endpoint: '/api/v1/threat-intelligence/intel-operations/api-management/export',
        format: 'json'
      });
    } catch (err) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-center h-64">
          <div className="text-lg">Loading Threat Intelligence APIs...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-4">
        <div className="flex items-center space-x-4 text-sm">
          <div className={`flex items-center ${connected ? 'text-green-600' : 'text-gray-500'}`}>
            ...
            {connected ? 'Threat Intelligence APIs System Online' : 'Threat Intelligence APIs System Offline'}
          </div>
          {stats && (
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">🌐 Threat Intelligence APIs</h1>
          <p className="text-gray-600 mt-2">API management for threat intelligence services</p>
        </div>
