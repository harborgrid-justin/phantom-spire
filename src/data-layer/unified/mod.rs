...
pub struct UnifiedDataStoreRegistry {
    stores: HashMap<String, Box<dyn UnifiedDataStore>>,
    primary_store: Option<String>,
}

impl UnifiedDataStoreRegistry {
    pub fn new() -> Self {
        Self {
            stores: HashMap::new(),
            primary_store: None,
    ...
    pub async fn register_store(&mut self, store: Box<dyn UnifiedDataStore>) -> UnifiedResult<()> {
        let store_id = store.store_id().to_string();
        
        // Set as primary if first store
        if self.primary_store.is_none() {
            self.primary_store = Some(store_id.clone());
        }
    ...
    /// Get a store by ID
    pub fn get_store(&self, store_id: &str) -> Option<&dyn UnifiedDataStore> {
        self.stores.get(store_id).map(|s| s.as_ref())
    }
    
    /// Get the primary store
    pub fn get_primary_store(&self) -> Option<&dyn UnifiedDataStore> {
        if let Some(primary_id) = &self.primary_store {
            self.get_store(primary_id)
        } else {
            None
        }
    }
    ...
    pub fn list_stores(&self) -> Vec<&str> {
        self.stores.keys().map(|s| s.as_str()).collect()
    }
    
    /// Cross-plugin query that searches across all stores
    pub async fn cross_plugin_query(&self, query: &UnifiedQuery, context: &UnifiedQueryContext) -> UnifiedResult<UnifiedQueryResult> {
        let mut all_records = Vec::new();
        let mut all_relationships = Vec::new();
        let mut total_query_time = 0u64;
        let mut total_count = 0usize;
        
        for store in self.stores.values() {
            match store.query_records(query, context).await {
                Ok(result) => {
                ...
                Err(e) => {
                    ...
                    eprintln!("Store {} query failed: {}", store.store_id(), e);
                }
            }
        }
        ...
        Ok(UnifiedQueryResult {
            ...
            pagination: None, // TODO: Implement cross-store pagination
        })
    }
}

impl Default for UnifiedDataStoreRegistry {
    fn default() -> Self {
        Self::new()
    }
}