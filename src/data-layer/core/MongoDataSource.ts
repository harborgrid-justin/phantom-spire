...
export class MongoDataSource extends BaseDataSource {
  ...
  private async executeSelectQuery(
    ...
    const data = documents.map(doc =>
      this.transformToDataRecord(doc, this.name)
    );
    ...
    return {
      ...
      metadata: {
        total,
        hasMore:
          (query.offset || 0) + (query.limit || documents.length) < total,
  ...
  private async executeAggregateQuery(
    ...
    return {
      data: results.map(result =>
        this.transformToDataRecord(result, this.name)
      ),
      metadata: {
  ...
  private async executeGraphQuery(
    ...
    const nodes = await this.findRelatedNodes(
      query.traversal.startNodes,
      relationships,
      query,
      context
    );
  ...
  private buildMongoQuery(
    ...
    if (query.projection) {
      options.projection = query.projection.reduce(
        (acc, field) => {
          acc[field] = 1;
          return acc;
        },
