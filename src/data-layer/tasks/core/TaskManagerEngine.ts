    ...
    try {
      ...
      if (query.ids) {
        filteredTasks = filteredTasks.filter(task =>
          query.ids!.includes(task.id)
        );
      }

      if (query.types) {
        filteredTasks = filteredTasks.filter(task =>
          query.types!.includes(task.type)
        );
      }

      if (query.statuses) {
        filteredTasks = filteredTasks.filter(task =>
          query.statuses!.includes(task.status)
        );
      }
      ...
      if (query.createdAfter) {
        filteredTasks = filteredTasks.filter(
          task => task.createdAt >= query.createdAfter!
        );
      }

      if (query.createdBefore) {
        filteredTasks = filteredTasks.filter(
          task => task.createdAt <= query.createdBefore!
        );
      }

      if (query.createdBy) {
        filteredTasks = filteredTasks.filter(
          task => task.createdBy === query.createdBy
        );
      }

      if (query.assignedTo) {
        filteredTasks = filteredTasks.filter(
          task => task.assignedTo === query.assignedTo
        );
      }
