...
const logger = new Logger('ProcedureAutomationWorkflowBusinessLogic');

interface ProcedureAutomationWorkflowAnalytics {
  totalCount: number;
...
export class ProcedureAutomationWorkflowBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<ProcedureAutomationWorkflowAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'procedures-intelligence',
        component: 'ProcedureAutomationWorkflow',
        itemId: data.id,
        status: data.status,
