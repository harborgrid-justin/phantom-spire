...
const logger = new Logger('ProcedureDocumentationCenterBusinessLogic');

interface ProcedureDocumentationCenterAnalytics {
  totalCount: number;
...
export class ProcedureDocumentationCenterBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<ProcedureDocumentationCenterAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'procedures-intelligence',
        component: 'ProcedureDocumentationCenter',
        itemId: data.id,
        status: data.status,
