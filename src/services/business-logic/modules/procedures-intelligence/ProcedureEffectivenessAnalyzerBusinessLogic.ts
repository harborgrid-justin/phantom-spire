...
const logger = new Logger('ProcedureEffectivenessAnalyzerBusinessLogic');

interface ProcedureEffectivenessAnalyzerAnalytics {
  totalCount: number;
...
export class ProcedureEffectivenessAnalyzerBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<ProcedureEffectivenessAnalyzerAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'procedures-intelligence',
        component: 'ProcedureEffectivenessAnalyzer',
        itemId: data.id,
        status: data.status,
