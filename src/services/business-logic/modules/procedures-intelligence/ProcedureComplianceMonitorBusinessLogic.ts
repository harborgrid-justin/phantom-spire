...
const logger = new Logger('ProcedureComplianceMonitorBusinessLogic');

interface ProcedureComplianceMonitorAnalytics {
  totalCount: number;
...
export class ProcedureComplianceMonitorBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<ProcedureComplianceMonitorAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'procedures-intelligence',
        component: 'ProcedureComplianceMonitor',
        itemId: data.id,
        status: data.status,
