...
const logger = new Logger('CommandControlAnalyzerBusinessLogic');

interface CommandControlAnalyzerAnalytics {
  totalCount: number;
...
export class CommandControlAnalyzerBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<CommandControlAnalyzerAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'tactics-analysis',
        component: 'CommandControlAnalyzer',
        itemId: data.id,
        status: data.status,
