...
const logger = new Logger('PersistenceTrackerBusinessLogic');

interface PersistenceTrackerAnalytics {
  totalCount: number;
...
export class PersistenceTrackerBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<PersistenceTrackerAnalytics> {
    try {
