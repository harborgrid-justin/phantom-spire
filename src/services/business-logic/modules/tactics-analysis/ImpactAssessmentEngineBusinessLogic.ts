...
const logger = new Logger('ImpactAssessmentEngineBusinessLogic');

interface ImpactAssessmentEngineAnalytics {
  totalCount: number;
...
export class ImpactAssessmentEngineBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<ImpactAssessmentEngineAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'tactics-analysis',
        component: 'ImpactAssessmentEngine',
        itemId: data.id,
        status: data.status,
