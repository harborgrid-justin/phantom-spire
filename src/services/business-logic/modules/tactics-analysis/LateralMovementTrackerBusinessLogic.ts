...
const logger = new Logger('LateralMovementTrackerBusinessLogic');

interface LateralMovementTrackerAnalytics {
  totalCount: number;
...
export class LateralMovementTrackerBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<LateralMovementTrackerAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'tactics-analysis',
        component: 'LateralMovementTracker',
        itemId: data.id,
        status: data.status,
