...
const logger = new Logger('PrivilegeEscalationDetectorBusinessLogic');

interface PrivilegeEscalationDetectorAnalytics {
  totalCount: number;
...
export class PrivilegeEscalationDetectorBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<PrivilegeEscalationDetectorAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'tactics-analysis',
        component: 'PrivilegeEscalationDetector',
        itemId: data.id,
        status: data.status,
