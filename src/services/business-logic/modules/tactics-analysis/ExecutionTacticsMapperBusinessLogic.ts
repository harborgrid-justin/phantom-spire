...
const logger = new Logger('ExecutionTacticsMapperBusinessLogic');

interface ExecutionTacticsMapperAnalytics {
  totalCount: number;
...
export class ExecutionTacticsMapperBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<ExecutionTacticsMapperAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'tactics-analysis',
        component: 'ExecutionTacticsMapper',
        itemId: data.id,
        status: data.status,
