...
const logger = new Logger('CredentialAccessMonitorBusinessLogic');

interface CredentialAccessMonitorAnalytics {
  totalCount: number;
...
export class CredentialAccessMonitorBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<CredentialAccessMonitorAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'tactics-analysis',
        component: 'CredentialAccessMonitor',
        itemId: data.id,
        status: data.status,
