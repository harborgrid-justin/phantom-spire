...
const logger = new Logger('InitialAccessAnalyzerBusinessLogic');

interface InitialAccessAnalyzerAnalytics {
  totalCount: number;
...
export class InitialAccessAnalyzerBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<InitialAccessAnalyzerAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'tactics-analysis',
        component: 'InitialAccessAnalyzer',
        itemId: data.id,
        status: data.status,
