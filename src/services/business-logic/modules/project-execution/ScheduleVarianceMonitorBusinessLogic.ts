/**
 * Schedule Variance Monitor Business Logic
 */
...
export class ScheduleVarianceMonitorBusinessLogic extends BusinessLogicBase {
  constructor() {
    super('scheduleVarianceMonitor', 'Schedule Variance Monitor', 'scheduling');
  }
  ...
  async processWorkflow(request: BusinessLogicRequest): Promise<BusinessLogicResponse> {
    try {
      ...
      return {
        ...
        metadata: {
          category: 'scheduling',
          module: 'schedule-variance-monitor'
        }
      };
    } catch (error) {
      return {
