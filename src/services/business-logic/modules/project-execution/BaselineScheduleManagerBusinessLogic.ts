/**
 * Baseline Schedule Manager Business Logic
 */
...
export class BaselineScheduleManagerBusinessLogic extends BusinessLogicBase {
  constructor() {
    super('baselineScheduleManager', 'Baseline Schedule Manager', 'scheduling');
  }
  ...
  async processWorkflow(request: BusinessLogicRequest): Promise<BusinessLogicResponse> {
    try {
      ...
      return {
        ...
        metadata: {
          category: 'scheduling',
          module: 'baseline-schedule-manager'
        }
      };
    } catch (error) {
      return {
