...
const logger = new Logger('TechniquePrioritizationEngineBusinessLogic');

interface TechniquePrioritizationEngineAnalytics {
  totalCount: number;
...
export class TechniquePrioritizationEngineBusinessLogic {
  private db: DatabaseConnection;
  private collectionName = 'techniques_mapping_ttp';
  ...
  async getAll(options: TTPQueryOptions = {}): Promise<{ items: TTPData[]; total: number }> {
    try {
      ...
      logger.debug('Getting all techniques mapping TTP items', { options });

      // Build filter query
      const filter: any = { category: 'techniques-mapping' };
      ...
      logger.info(`Retrieved ${items.length} techniques mapping TTP items`);
      return { items, total };

    } catch (error) {
      logger.error('Failed to get techniques mapping TTP items', { error: error.message, options });
      throw error;
    }
  }
  ...
  async getById(id: string): Promise<TTPData | null> {
    try {
      logger.debug('Getting techniques mapping TTP item by ID', { id });

      const item = await this.db.findOne(this.collectionName, { id });
      
      if (!item) {
        logger.warn('techniques mapping TTP item not found', { id });
        return null;
      }

      logger.info('Retrieved techniques mapping TTP item', { id, name: item.name });
      return item;

    } catch (error) {
      logger.error('Failed to get techniques mapping TTP item', { error: error.message, id });
      throw error;
    }
  }
  ...
  async create(input: TTPCreateInput, userId: string): Promise<TTPData> {
    try {
      logger.info('Creating new techniques mapping TTP item', { input, userId });

      const now = new Date();
      const ttpData: TTPData = {
        id: this.generateId(),
        ...input,
        category: 'techniques-mapping',
      ...
      logger.info('Successfully created techniques mapping TTP item', { id: ttpData.id, name: ttpData.name });
      return ttpData;

    } catch (error) {
      logger.error('Failed to create techniques mapping TTP item', { error: error.message, input, userId });
      throw error;
    }
  }
  ...
  async update(id: string, input: TTPUpdateInput, userId: string): Promise<TTPData | null> {
    try {
      logger.info('Updating techniques mapping TTP item', { id, input, userId });
      ...
      if (result.matchedCount === 0) {
        logger.warn('techniques mapping TTP item not found for update', { id });
        return null;
      }
      ...
      logger.info('Successfully updated techniques mapping TTP item', { id });
      return updatedItem;

    } catch (error) {
      logger.error('Failed to update techniques mapping TTP item', { error: error.message, id, input, userId });
      throw error;
    }
  }
  ...
  async delete(id: string, userId: string): Promise<boolean> {
    try {
      logger.info('Deleting techniques mapping TTP item', { id, userId });

      const item = await this.getById(id);
      if (!item) {
        logger.warn('techniques mapping TTP item not found for deletion', { id });
        return false;
      }
      ...
      if (result.deletedCount > 0) {
        ...
        logger.info('Successfully deleted techniques mapping TTP item', { id });
        return true;
      }
      ...
      logger.error('Failed to delete techniques mapping TTP item', { error: error.message, id, userId });
      throw error;
    }
  }
   ...
   */
  async getAnalytics(): Promise<TechniquePrioritizationEngineAnalytics> {
    try {
      logger.debug('Getting techniques mapping TTP analytics');
      ...
      const [
        ...
        this.db.count(this.collectionName, { category: 'techniques-mapping' }),
        this.db.aggregate(this.collectionName, [
          { $match: { category: 'techniques-mapping' } },
      ...
      logger.error('Failed to get techniques mapping TTP analytics', { error: error.message });
      throw error;
    }
  }
  ...
  async bulkUpdate(
    ...
    try {
      logger.info(`Bulk updating ${ids.length} techniques mapping TTP items`, { userId, update });
      ...
      logger.info(`Successfully bulk updated ${result.modifiedCount} techniques mapping TTP items`);
      return result.modifiedCount;

    } catch (error) {
      logger.error('Failed to bulk update techniques mapping TTP items', { error: error.message, ids, update, userId });
      throw error;
    }
  }
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'techniques-mapping',
        component: 'TechniquePrioritizationEngine',
        itemId: data.id,
        status: data.status,
