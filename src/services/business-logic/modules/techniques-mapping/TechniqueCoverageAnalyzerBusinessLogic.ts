...
const logger = new Logger('TechniqueCoverageAnalyzerBusinessLogic');

interface TechniqueCoverageAnalyzerAnalytics {
  totalCount: number;
...
export class TechniqueCoverageAnalyzerBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<TechniqueCoverageAnalyzerAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'techniques-mapping',
        component: 'TechniqueCoverageAnalyzer',
        itemId: data.id,
        status: data.status,
