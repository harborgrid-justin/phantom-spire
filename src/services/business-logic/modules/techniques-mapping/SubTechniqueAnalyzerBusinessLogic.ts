...
const logger = new Logger('SubTechniqueAnalyzerBusinessLogic');

interface SubTechniqueAnalyzerAnalytics {
  totalCount: number;
...
export class SubTechniqueAnalyzerBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<SubTechniqueAnalyzerAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'techniques-mapping',
        component: 'SubTechniqueAnalyzer',
        itemId: data.id,
        status: data.status,
