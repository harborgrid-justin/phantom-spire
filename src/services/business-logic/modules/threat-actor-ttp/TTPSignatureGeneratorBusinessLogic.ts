...
const logger = new Logger('TTPSignatureGeneratorBusinessLogic');

interface TTPSignatureGeneratorAnalytics {
  totalCount: number;
...
export class TTPSignatureGeneratorBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<TTPSignatureGeneratorAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'threat-actor-ttp',
        component: 'TTPSignatureGenerator',
        itemId: data.id,
        status: data.status,
