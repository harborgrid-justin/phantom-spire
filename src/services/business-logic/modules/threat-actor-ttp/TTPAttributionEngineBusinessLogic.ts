...
const logger = new Logger('TTPAttributionEngineBusinessLogic');

interface TTPAttributionEngineAnalytics {
  totalCount: number;
...
export class TTPAttributionEngineBusinessLogic {
   ...
   */
  async getAnalytics(): Promise<TTPAttributionEngineAnalytics> {
    try {
  ...
  private async recordMetrics(operation: string, data: TTPData): Promise<void> {
    try {
      const metrics = {
        operation,
        category: 'threat-actor-ttp',
        component: 'TTPAttributionEngine',
        itemId: data.id,
        status: data.status,
