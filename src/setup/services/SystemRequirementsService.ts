...
export class SystemRequirementsService {
  private async checkNodeVersion(): Promise<SystemRequirement> {
    ...
    const passed =
      curMajor > reqMajor || (curMajor === reqMajor && curMinor >= reqMinor);
  ...
  private async checkMemory(): Promise<SystemRequirement> {
    ...
    const totalMemGB =
      Math.round((totalMem.rss / 1024 / 1024 / 1024) * 100) / 100;
  ...
  private async checkDirectoryStructure(): Promise<SystemRequirement> {
    const requiredDirectories = ['src', 'logs', 'uploads', 'backups', 'docker'];
  ...
  public async performSystemCheck(): Promise<SystemCheckResult> {
    ...
    if (warnings.length > 0) {
      recommendations.push(
        `Address ${warnings.length} warning(s) for optimal performance`
      );
    }
    if (
      checks.find(check => check.name === 'Available Memory' && !check.passed)
    ) {
      recommendations.push(
    ...
    console.log(
      `- ${checks.filter(c => c.passed).length}/${checks.length} requirements met`
    );
  ...
  public async generateSystemReport(): Promise<string> {
    ...
    let report =
      '# Phantom Spire CTI Platform - System Requirements Report\n\n';
