...
export interface LineEvaluation {
  lineNumber: number;
  content: string;
  category: 'security' | 'best-practice' | 'error-handling' | 'performance' | 'maintainability' | 'correctness';
...
export class InstallationEvaluationService {
  ...
  public async evaluateInstallationScripts(): Promise<InstallationEvaluationResult> {
    ...
    for (const scriptPath of scriptPaths) {
      try {
        ...
        console.warn(`Could not evaluate script ${scriptPath}:`, (error as Error).message);
      }
    }

    const overallScore = scriptEvaluations.length > 0
      ? scriptEvaluations.reduce((sum, evaluation) => sum + evaluation.overallScore, 0) / scriptEvaluations.length
      : 0;

    const complianceChecks = this.performComplianceChecks(scriptEvaluations);
    const globalRecommendations = this.generateGlobalRecommendations(scriptEvaluations, complianceChecks);
  ...
  private async evaluateScript(scriptPath: string): Promise<ScriptEvaluation> {
    ...
    const criticalIssues = lineEvaluations.filter(e => !e.passed && e.severity === 'critical').length;
    const highIssues = lineEvaluations.filter(e => !e.passed && e.severity === 'high').length;
    const mediumIssues = lineEvaluations.filter(e => !e.passed && e.severity === 'medium').length;
    const lowIssues = lineEvaluations.filter(e => !e.passed && e.severity === 'low').length;
    
    const totalIssues = criticalIssues + highIssues + mediumIssues + lowIssues;
    const issueScore = Math.max(0, 100 - (criticalIssues * 30 + highIssues * 20 + mediumIssues * 10 + lowIssues * 5));
  ...
  private evaluateLine(line: string, lineNumber: number): LineEvaluation | null {
  ...
  private evaluateLineSecurity(line: string, lineNumber: number): LineEvaluation[] {
    const evaluations: LineEvaluation[] = [];
    ...
    if (this.securityPatterns.unsafeDownloads.test(line)) {
      evaluations.push({
        ...
        reasoning: 'This pattern allows arbitrary code execution and is a major security risk'
      });
    }
    
    // Check for HTTP downloads
    if (this.securityPatterns.httpDownloads.test(line) && !this.securityPatterns.httpsDownloads.test(line)) {
      evaluations.push({
        lineNumber,
        content: line,
        category: 'security',
        ...
        reasoning: 'HTTP downloads can be intercepted and modified by attackers'
      });
    }
    
    // Check for sudo without validation
    if (this.securityPatterns.sudoWithoutValidation.test(line) && !line.includes('systemctl')) {
      evaluations.push({
        lineNumber,
        content: line,
        category: 'security',
  ...
  private evaluateLineBestPractices(line: string, lineNumber: number): LineEvaluation[] {
    const evaluations: LineEvaluation[] = [];
    
    // Check for unquoted variables
    if (this.bestPracticePatterns.unquotedVariables.test(line) && !line.includes('$(') && !line.includes('`')) {
      evaluations.push({
        lineNumber,
        content: line,
        ...
        reasoning: 'Unquoted variables can cause unexpected behavior with spaces or special characters'
      });
    }
  ...
  private evaluateLineErrorHandling(line: string, lineNumber: number): LineEvaluation[] {
    const evaluations: LineEvaluation[] = [];
    
    // Check for commands without error handling
    if (line.startsWith('curl ') || line.startsWith('wget ') || line.startsWith('npm ') || line.startsWith('apt-get ')) {
  ...
  private generateScriptSummary(lineEvaluations: LineEvaluation[], content: string): {
    strengths: string[];
    weaknesses: string[];
    recommendations: string[];
  } {
    ...
    if (content.includes('function ') || /^[a-zA-Z_][a-zA-Z0-9_]*\(\)/.test(content)) {
      strengths.push('Well-organized code with functions');
    }
    
    // Analyze weaknesses from evaluations
    const criticalIssues = lineEvaluations.filter(e => !e.passed && e.severity === 'critical');
    const securityIssues = lineEvaluations.filter(e => !e.passed && e.category === 'security');
    
    if (criticalIssues.length > 0) {
      weaknesses.push(`${criticalIssues.length} critical security issues found`);
    }
    ...
    if (!this.errorHandlingPatterns.hasGlobalErrorHandling.test(content)) {
      recommendations.push('Add global error handling with "set -e" and trap statements');
    }
  ...
  private performComplianceChecks(scriptEvaluations: ScriptEvaluation[]): {
    ...
    const allContent = scriptEvaluations.map(se => {
      return se.lineEvaluations.map(le => le.content).join('\n');
    }).join('\n');
    
    const allScriptContents = scriptEvaluations.map(se => se.fileName + '\n' + 
      se.lineEvaluations.map(le => le.content).join('\n')).join('\n\n');
    
    return {
      hasErrorHandling: this.errorHandlingPatterns.hasGlobalErrorHandling.test(allScriptContents),
      hasInputValidation: this.securityPatterns.inputValidation.test(allScriptContents),
      hasSecurityMeasures: this.securityPatterns.userValidation.test(allScriptContents),
      hasLogging: this.bestPracticePatterns.logging.test(allScriptContents),
      hasCleanup: allScriptContents.includes('cleanup') || allScriptContents.includes('rm -rf')
    };
  }
  ...
  private generateGlobalRecommendations(
    ...
    const totalCritical = scriptEvaluations.reduce((sum, evaluation) => sum + evaluation.criticalIssues, 0);
    const totalHigh = scriptEvaluations.reduce((sum, evaluation) => sum + evaluation.highIssues, 0);
    const avgScore = scriptEvaluations.reduce((sum, evaluation) => sum + evaluation.overallScore, 0) / scriptEvaluations.length;
    
    if (totalCritical > 0) {
      recommendations.push(`🚨 Address ${totalCritical} critical security vulnerabilities immediately`);
    }
    
    if (totalHigh > 2) {
      recommendations.push(`⚠️  Review and fix ${totalHigh} high-priority issues`);
    }
    
    if (avgScore < 70) {
      recommendations.push('📈 Overall installation script quality is below recommended threshold (70%)');
    }
    
    if (!complianceChecks.hasErrorHandling) {
      recommendations.push('🛡️  Implement comprehensive error handling across all scripts');
    }
    
    if (!complianceChecks.hasInputValidation) {
      recommendations.push('✅ Add input validation for all user inputs and parameters');
    }
    
    if (!complianceChecks.hasSecurityMeasures) {
      recommendations.push('🔒 Implement security measures like user privilege validation');
    }
    
    recommendations.push('📋 Consider implementing automated testing for installation scripts');
    recommendations.push('🔄 Set up continuous integration to validate installation process changes');
  ...
  public async generateDetailedReport(): Promise<string> {
    ...
    if (evaluation.overallScore >= 80) {
      report += '✅ **Status:** Good - Installation scripts meet quality standards\n';
    } else if (evaluation.overallScore >= 60) {
      report += '⚠️ **Status:** Needs Improvement - Several issues require attention\n';
    } else {
      report += '❌ **Status:** Poor - Significant issues need immediate attention\n';
    }
    report += '\n';
    ...
    for (const scriptEval of evaluation.scriptEvaluations) {
      ...
      const criticalAndHigh = scriptEval.lineEvaluations.filter(
        le => !le.passed && (le.severity === 'critical' || le.severity === 'high')
      );
