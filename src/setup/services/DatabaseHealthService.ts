...
export class DatabaseHealthService {
  ...
  private async checkPostgreSQLConnection(
    startTime: number
  ): Promise<DatabaseConnectionResult> {
  ...
  async performHealthCheck(): Promise<DatabaseHealthCheck> {
    ...
    const connectedCount = Object.values(databaseResults).filter(
      r => r.connected
    ).length;
    const totalResponseTime = Object.values(databaseResults).reduce(
      (sum, r) => sum + r.responseTime,
      0
    );
    const averageResponseTime = Math.round(
      totalResponseTime / databases.length
    );
    ...
    console.log(
      `Database health check completed: ${connectedCount}/${databases.length} databases connected`
    );
