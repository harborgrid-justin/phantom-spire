...
export class SetupService {
  ...
  public async getSetupStatus(): Promise<SetupStatus> {
    ...
    return {
      ...
      lastActivity: setupState?.lastActivity
        ? new Date(setupState?.lastActivity)
        : null,
    };
  }
  ...
  private getStepMessage(
    ...
    if (completed) {
      switch (step) {
        case 'system-check':
          return 'System requirements verified';
        case 'database-connections':
          return 'All databases connected successfully';
        case 'admin-user':
          return 'Administrator user created';
        case 'system-config':
          return 'System configuration saved';
        case 'integrations':
          return 'External integrations configured';
        case 'finalization':
      ...
      switch (step) {
        ...
        case 'database-connections':
          return 'Testing database connections...';
        case 'admin-user':
          return 'Creating administrator user...';
        case 'system-config':
          return 'Configuring system settings...';
        case 'integrations':
          return 'Setting up integrations...';
        case 'finalization':
      ...
      switch (step) {
        case 'system-check':
          return 'System requirements check pending';
        case 'database-connections':
          return 'Database connection tests pending';
        case 'admin-user':
          return 'Administrator user creation pending';
        case 'system-config':
          return 'System configuration pending';
        case 'integrations':
          return 'Integration setup pending';
        case 'finalization':
          return 'Final setup pending';
        default:
          return 'Pending';
      }
    }
  ...
  private async testMISPIntegration(
    config: any,
    startTime: number
  ): Promise<IntegrationTestResult> {
    ...
    try {
      const response = await axios.get(
        `${config.url.replace(/\/+$/, '')}/servers/getPyMISPVersion.json`,
        {
          headers: {
  ...
  private async testOTXIntegration(
    config: any,
    startTime: number
  ): Promise<IntegrationTestResult> {
    if (!config.apiKey) {
  ...
  private async testVirusTotalIntegration(
    config: any,
    startTime: number
  ): Promise<IntegrationTestResult> {
    if (!config.apiKey) {
  ...
  public async getDatabaseSchemaStatus(): Promise<any> {
    ...
    try {
      ...
      console.warn(
        'Could not check database schemas:',
  ...
  public async finalizeSetup(config: SetupConfiguration): Promise<any> {
    ...
    if (!healthCheck.overall.healthy) {
      throw new Error(
        'Database health check failed. Ensure all databases are running and accessible.'
      );
    }
  ...
  private async saveIntegrationConfiguration(
  ...
  private async initializeDataStructures(): Promise<void> {
    ...
    try {
      ...
      const workflowCount = await connection.db
  ...
  private async createDefaultWorkflows(
    ...
    const defaultWorkflows = [
      {
        ...
        description:
          'Automatically enrich threat indicators with external intelligence sources',
        definition: {
          steps: [
  ...
  private async generateSetupReport(
    ...
    const healthCheck = await this.databaseHealthService.performHealthCheck();
    const systemCheck =
      await this.systemRequirementsService.performSystemCheck();
    ...
    Object.entries(config.integrationConfig).forEach(
      ([integration, config]) => {
