 ...
 * Fortune 100-Grade persistence layer for workflow definitions and instances
 */
...
export class MongoWorkflowRepository implements IWorkflowRepository {
  ...
  public async getDefinitions(filters?: any): Promise<IWorkflowDefinition[]> {
    try {
      ...
      return definitions.map(def => def.toObject() as IWorkflowDefinition);
    } catch (error) {
  ...
  public async getInstance(id: string): Promise<IWorkflowInstance> {
    try {
      const instance = await WorkflowInstance.findOne({ id });
      
      if (!instance) {
        throw new Error(`Workflow instance not found: ${id}`);
      }

      return instance.toObject() as IWorkflowInstance;
    } catch (error) {
  ...
  public async getInstances(filters?: IWorkflowInstanceFilter): Promise<IWorkflowInstance[]> {
    try {
      ...
      return instances.map(instance => instance.toObject() as IWorkflowInstance);
    } catch (error) {
