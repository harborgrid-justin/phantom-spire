...
export const QualityValidation: React.FC = () => {
  ...
  const {
    ...
    hasErrors,
    refresh
  } = useServicePage('ingestion-quality-validation');
  ...
  useEffect(() => {
    addUIUXEvaluation({
      page: 'QualityValidation',
      component: 'MainContent',
      loadTime: performance.now(),
      interactionPath: ['data-management', 'ingestion', 'quality-validation']
    });
  }, []);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/data-management/ingestion/quality-validation');
        if (!response.ok) {
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} lg={8}>
          <Paper elevation={2} sx={{ p: 3 }}>
            ...
            <Typography variant="body1" color="textSecondary" paragraph>
              This is the main content area for ✅ Quality Validation Center. 
