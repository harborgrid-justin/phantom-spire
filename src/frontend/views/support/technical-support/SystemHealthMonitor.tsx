...
interface SystemHealthMonitorData {
  id: string;
  title: string;
...
interface SystemHealthMonitorMetrics {
  totalItems: number;
  activeItems: number;
...
const SystemHealthMonitor: React.FC = () => {
  const [data, setData] = useState<SystemHealthMonitorData[]>([]);
  const [metrics, setMetrics] = useState<SystemHealthMonitorMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<SystemHealthMonitorData | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/system-health-monitor');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<SystemHealthMonitorData>) => {
    try {
      const response = await fetch('/api/v1/support/system-health-monitor', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<SystemHealthMonitorData>) => {
    try {
      const response = await fetch(`/api/v1/support/system-health-monitor/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
