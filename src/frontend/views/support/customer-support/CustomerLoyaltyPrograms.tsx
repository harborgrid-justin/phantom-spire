...
interface CustomerLoyaltyProgramsData {
  id: string;
  title: string;
...
interface CustomerLoyaltyProgramsMetrics {
  totalItems: number;
  activeItems: number;
...
const CustomerLoyaltyPrograms: React.FC = () => {
  const [data, setData] = useState<CustomerLoyaltyProgramsData[]>([]);
  const [metrics, setMetrics] = useState<CustomerLoyaltyProgramsMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<CustomerLoyaltyProgramsData | null>(null);
  ...
  useEffect(() => {
    addUIUXEvaluation({
      componentName: 'CustomerLoyaltyPrograms',
      category: 'customer-support',
      metrics: {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/customer-loyalty-programs');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<CustomerLoyaltyProgramsData>) => {
    try {
      const response = await fetch('/api/v1/support/customer-loyalty-programs', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<CustomerLoyaltyProgramsData>) => {
    try {
      const response = await fetch(`/api/v1/support/customer-loyalty-programs/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
...
export default CustomerLoyaltyPrograms;
