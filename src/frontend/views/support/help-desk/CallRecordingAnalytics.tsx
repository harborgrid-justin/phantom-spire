...
interface CallRecordingAnalyticsData {
  id: string;
  title: string;
...
interface CallRecordingAnalyticsMetrics {
  totalItems: number;
  activeItems: number;
...
const CallRecordingAnalytics: React.FC = () => {
  const [data, setData] = useState<CallRecordingAnalyticsData[]>([]);
  const [metrics, setMetrics] = useState<CallRecordingAnalyticsMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<CallRecordingAnalyticsData | null>(null);
  ...
  useEffect(() => {
    addUIUXEvaluation({
      componentName: 'CallRecordingAnalytics',
      category: 'help-desk',
      metrics: {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/call-recording-analytics');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<CallRecordingAnalyticsData>) => {
    try {
      const response = await fetch('/api/v1/support/call-recording-analytics', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<CallRecordingAnalyticsData>) => {
    try {
      const response = await fetch(`/api/v1/support/call-recording-analytics/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
