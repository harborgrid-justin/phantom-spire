...
interface AgentPerformanceAnalyticsData {
  id: string;
  title: string;
...
interface AgentPerformanceAnalyticsMetrics {
  totalItems: number;
  activeItems: number;
...
const AgentPerformanceAnalytics: React.FC = () => {
  const [data, setData] = useState<AgentPerformanceAnalyticsData[]>([]);
  const [metrics, setMetrics] = useState<AgentPerformanceAnalyticsMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<AgentPerformanceAnalyticsData | null>(null);
  ...
  useEffect(() => {
    addUIUXEvaluation({
      componentName: 'AgentPerformanceAnalytics',
      category: 'help-desk',
      metrics: {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/agent-performance-analytics');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<AgentPerformanceAnalyticsData>) => {
    try {
      const response = await fetch('/api/v1/support/agent-performance-analytics', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<AgentPerformanceAnalyticsData>) => {
    try {
      const response = await fetch(`/api/v1/support/agent-performance-analytics/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
