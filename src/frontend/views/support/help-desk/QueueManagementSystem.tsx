...
interface QueueManagementSystemData {
  id: string;
  title: string;
...
interface QueueManagementSystemMetrics {
  totalItems: number;
  activeItems: number;
...
const QueueManagementSystem: React.FC = () => {
  const [data, setData] = useState<QueueManagementSystemData[]>([]);
  const [metrics, setMetrics] = useState<QueueManagementSystemMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<QueueManagementSystemData | null>(null);
  ...
  useEffect(() => {
    addUIUXEvaluation({
      componentName: 'QueueManagementSystem',
      category: 'help-desk',
      metrics: {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/queue-management-system');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<QueueManagementSystemData>) => {
    try {
      const response = await fetch('/api/v1/support/queue-management-system', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<QueueManagementSystemData>) => {
    try {
      const response = await fetch(`/api/v1/support/queue-management-system/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
