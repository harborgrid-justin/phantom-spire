...
interface AgentWorkbenchMetrics {
  totalItems: number;
  activeItems: number;
...
const AgentWorkbench: React.FC = () => {
  const [data, setData] = useState<AgentWorkbenchData[]>([]);
  const [metrics, setMetrics] = useState<AgentWorkbenchMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<AgentWorkbenchData | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/agent-workbench');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<AgentWorkbenchData>) => {
    try {
      const response = await fetch('/api/v1/support/agent-workbench', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<AgentWorkbenchData>) => {
    try {
      const response = await fetch(`/api/v1/support/agent-workbench/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
