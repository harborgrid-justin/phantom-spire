...
interface KnowledgeBasePortalData {
  id: string;
  title: string;
...
interface KnowledgeBasePortalMetrics {
  totalItems: number;
  activeItems: number;
...
const KnowledgeBasePortal: React.FC = () => {
  const [data, setData] = useState<KnowledgeBasePortalData[]>([]);
  const [metrics, setMetrics] = useState<KnowledgeBasePortalMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<KnowledgeBasePortalData | null>(null);
  ...
  useEffect(() => {
    addUIUXEvaluation({
      componentName: 'KnowledgeBasePortal',
      category: 'knowledge-management',
      metrics: {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/knowledge-base-portal');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<KnowledgeBasePortalData>) => {
    try {
      const response = await fetch('/api/v1/support/knowledge-base-portal', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<KnowledgeBasePortalData>) => {
    try {
      const response = await fetch(`/api/v1/support/knowledge-base-portal/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
