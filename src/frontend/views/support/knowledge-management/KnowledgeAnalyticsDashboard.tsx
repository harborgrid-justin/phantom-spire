...
interface KnowledgeAnalyticsDashboardData {
  id: string;
  title: string;
...
interface KnowledgeAnalyticsDashboardMetrics {
  totalItems: number;
  activeItems: number;
...
const KnowledgeAnalyticsDashboard: React.FC = () => {
  const [data, setData] = useState<KnowledgeAnalyticsDashboardData[]>([]);
  const [metrics, setMetrics] = useState<KnowledgeAnalyticsDashboardMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<KnowledgeAnalyticsDashboardData | null>(null);
  ...
  useEffect(() => {
    addUIUXEvaluation({
      componentName: 'KnowledgeAnalyticsDashboard',
      category: 'knowledge-management',
      metrics: {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/knowledge-analytics-dashboard');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<KnowledgeAnalyticsDashboardData>) => {
    try {
      const response = await fetch('/api/v1/support/knowledge-analytics-dashboard', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<KnowledgeAnalyticsDashboardData>) => {
    try {
      const response = await fetch(`/api/v1/support/knowledge-analytics-dashboard/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
