...
interface ContentApprovalWorkflowData {
  id: string;
  title: string;
...
interface ContentApprovalWorkflowMetrics {
  totalItems: number;
  activeItems: number;
...
const ContentApprovalWorkflow: React.FC = () => {
  const [data, setData] = useState<ContentApprovalWorkflowData[]>([]);
  const [metrics, setMetrics] = useState<ContentApprovalWorkflowMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<ContentApprovalWorkflowData | null>(null);
  ...
  useEffect(() => {
    addUIUXEvaluation({
      componentName: 'ContentApprovalWorkflow',
      category: 'knowledge-management',
      metrics: {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/content-approval-workflow');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<ContentApprovalWorkflowData>) => {
    try {
      const response = await fetch('/api/v1/support/content-approval-workflow', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<ContentApprovalWorkflowData>) => {
    try {
      const response = await fetch(`/api/v1/support/content-approval-workflow/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
