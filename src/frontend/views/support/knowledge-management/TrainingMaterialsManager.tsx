...
interface TrainingMaterialsManagerData {
  id: string;
  title: string;
...
interface TrainingMaterialsManagerMetrics {
  totalItems: number;
  activeItems: number;
...
const TrainingMaterialsManager: React.FC = () => {
  const [data, setData] = useState<TrainingMaterialsManagerData[]>([]);
  const [metrics, setMetrics] = useState<TrainingMaterialsManagerMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<TrainingMaterialsManagerData | null>(null);
  ...
  useEffect(() => {
    addUIUXEvaluation({
      componentName: 'TrainingMaterialsManager',
      category: 'knowledge-management',
      metrics: {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/training-materials-manager');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<TrainingMaterialsManagerData>) => {
    try {
      const response = await fetch('/api/v1/support/training-materials-manager', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<TrainingMaterialsManagerData>) => {
    try {
      const response = await fetch(`/api/v1/support/training-materials-manager/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
