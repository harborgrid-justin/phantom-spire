...
interface KnowledgeGapAnalyzerData {
  id: string;
  title: string;
...
interface KnowledgeGapAnalyzerMetrics {
  totalItems: number;
  activeItems: number;
...
const KnowledgeGapAnalyzer: React.FC = () => {
  const [data, setData] = useState<KnowledgeGapAnalyzerData[]>([]);
  const [metrics, setMetrics] = useState<KnowledgeGapAnalyzerMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<KnowledgeGapAnalyzerData | null>(null);
  ...
  useEffect(() => {
    addUIUXEvaluation({
      componentName: 'KnowledgeGapAnalyzer',
      category: 'knowledge-management',
      metrics: {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/knowledge-gap-analyzer');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<KnowledgeGapAnalyzerData>) => {
    try {
      const response = await fetch('/api/v1/support/knowledge-gap-analyzer', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<KnowledgeGapAnalyzerData>) => {
    try {
      const response = await fetch(`/api/v1/support/knowledge-gap-analyzer/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
