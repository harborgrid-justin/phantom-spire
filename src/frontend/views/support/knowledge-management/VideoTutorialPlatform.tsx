...
interface VideoTutorialPlatformData {
  id: string;
  title: string;
...
interface VideoTutorialPlatformMetrics {
  totalItems: number;
  activeItems: number;
...
const VideoTutorialPlatform: React.FC = () => {
  const [data, setData] = useState<VideoTutorialPlatformData[]>([]);
  const [metrics, setMetrics] = useState<VideoTutorialPlatformMetrics | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [selectedItem, setSelectedItem] = useState<VideoTutorialPlatformData | null>(null);
  ...
  useEffect(() => {
    addUIUXEvaluation({
      componentName: 'VideoTutorialPlatform',
      category: 'knowledge-management',
      metrics: {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const response = await fetch('/api/v1/support/video-tutorial-platform');
        if (!response.ok) {
  ...
  const handleCreateItem = useCallback(async (itemData: Partial<VideoTutorialPlatformData>) => {
    try {
      const response = await fetch('/api/v1/support/video-tutorial-platform', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateItem = useCallback(async (id: string, updates: Partial<VideoTutorialPlatformData>) => {
    try {
      const response = await fetch(`/api/v1/support/video-tutorial-platform/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
...
export default VideoTutorialPlatform;
