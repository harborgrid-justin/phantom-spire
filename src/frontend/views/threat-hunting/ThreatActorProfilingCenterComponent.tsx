...
interface ThreatActorProfilingCenterData {
  id: string;
  name: string;
...
interface ThreatActorProfilingCenterProps {
  onRefresh?: () => void;
  onExport?: () => void;
}

const ThreatActorProfilingCenter: React.FC<ThreatActorProfilingCenterProps> = ({ 
  onRefresh, 
  onExport 
}) => {
  const [data, setData] = useState<ThreatActorProfilingCenterData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedTab, setSelectedTab] = useState(0);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<ThreatActorProfilingCenterData | null>(null);

  // Mock data for demonstration
  const mockData: ThreatActorProfilingCenterData[] = [
    {
      id: '1',
...
export default ThreatActorProfilingCenter;
