...
interface CampaignTrackingSystemData {
  id: string;
  name: string;
...
const CampaignTrackingSystem: React.FC<CampaignTrackingSystemProps> = ({ 
  onRefresh, 
  onExport 
}) => {
  const [data, setData] = useState<CampaignTrackingSystemData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedTab, setSelectedTab] = useState(0);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<CampaignTrackingSystemData | null>(null);

  // Mock data for demonstration
  const mockData: CampaignTrackingSystemData[] = [
    {
      id: '1',
...
export default CampaignTrackingSystem;
