...
interface IOCTrackingDashboardData {
  id: string;
  name: string;
...
interface IOCTrackingDashboardProps {
  onRefresh?: () => void;
  onExport?: () => void;
}

const IOCTrackingDashboard: React.FC<IOCTrackingDashboardProps> = ({ 
  onRefresh, 
  onExport 
}) => {
  const [data, setData] = useState<IOCTrackingDashboardData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedTab, setSelectedTab] = useState(0);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<IOCTrackingDashboardData | null>(null);

  // Mock data for demonstration
  const mockData: IOCTrackingDashboardData[] = [
    {
      id: '1',
...
export default IOCTrackingDashboard;
