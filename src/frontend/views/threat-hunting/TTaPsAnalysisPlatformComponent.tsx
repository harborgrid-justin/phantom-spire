...
interface TTaPsAnalysisPlatformData {
  id: string;
  name: string;
...
interface TTaPsAnalysisPlatformProps {
  onRefresh?: () => void;
  onExport?: () => void;
}

const TTaPsAnalysisPlatform: React.FC<TTaPsAnalysisPlatformProps> = ({ 
  onRefresh, 
  onExport 
}) => {
  const [data, setData] = useState<TTaPsAnalysisPlatformData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedTab, setSelectedTab] = useState(0);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<TTaPsAnalysisPlatformData | null>(null);

  // Mock data for demonstration
  const mockData: TTaPsAnalysisPlatformData[] = [
    {
      id: '1',
...
export default TTaPsAnalysisPlatform;
