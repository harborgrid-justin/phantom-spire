...
interface BehavioralAnalysisEngineData {
  id: string;
  name: string;
...
interface BehavioralAnalysisEngineProps {
  onRefresh?: () => void;
  onExport?: () => void;
}

const BehavioralAnalysisEngine: React.FC<BehavioralAnalysisEngineProps> = ({ 
  onRefresh, 
  onExport 
}) => {
  const [data, setData] = useState<BehavioralAnalysisEngineData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedTab, setSelectedTab] = useState(0);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<BehavioralAnalysisEngineData | null>(null);

  // Mock data for demonstration
  const mockData: BehavioralAnalysisEngineData[] = [
    {
      id: '1',
