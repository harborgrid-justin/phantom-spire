...
interface HuntingMetricsDashboardData {
  id: string;
  name: string;
...
interface HuntingMetricsDashboardProps {
  onRefresh?: () => void;
  onExport?: () => void;
}

const HuntingMetricsDashboard: React.FC<HuntingMetricsDashboardProps> = ({ 
  onRefresh, 
  onExport 
}) => {
  const [data, setData] = useState<HuntingMetricsDashboardData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedTab, setSelectedTab] = useState(0);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<HuntingMetricsDashboardData | null>(null);

  // Mock data for demonstration
  const mockData: HuntingMetricsDashboardData[] = [
    {
      id: '1',
...
export default HuntingMetricsDashboard;
