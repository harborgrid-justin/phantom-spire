...
interface AdvancedHuntingQueryBuilderData {
  id: string;
  name: string;
...
interface AdvancedHuntingQueryBuilderProps {
  onRefresh?: () => void;
  onExport?: () => void;
}

const AdvancedHuntingQueryBuilder: React.FC<AdvancedHuntingQueryBuilderProps> = ({ 
  onRefresh, 
  onExport 
}) => {
  const [data, setData] = useState<AdvancedHuntingQueryBuilderData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedTab, setSelectedTab] = useState(0);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [selectedItem, setSelectedItem] = useState<AdvancedHuntingQueryBuilderData | null>(null);

  // Mock data for demonstration
  const mockData: AdvancedHuntingQueryBuilderData[] = [
    {
      id: '1',
...
export default AdvancedHuntingQueryBuilder;
