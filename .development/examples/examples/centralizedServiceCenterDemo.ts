...
async function runCentralizedServiceCenterDemo(): Promise<void> {
  ...
  try {
    ...
    console.log(
      '🔗 All platform modules are linked and accessible through unified interface'
    );
  } catch (error) {
...
async function demonstrateServiceDiscovery(): Promise<void> {
  ...
  const intelligenceServices =
    await centralizedServiceCenter.findServicesByCapability('threat-analysis');
  console.log(`  - Threat analysis services: ${intelligenceServices.length}`);

  const analyticsServices =
    await centralizedServiceCenter.findServicesByCapability('analytics');
  console.log(`  - Analytics services: ${analyticsServices.length}`);

  const workflowServices =
    await centralizedServiceCenter.findServicesByCapability(
...
async function demonstrateUnifiedAPIAccess(): Promise<void> {
  ...
  for (const operation of operations) {
    console.log(
      `🔄 Executing ${operation.operation} on ${operation.serviceId}...`
    );
    ...
    if (result.success) {
      console.log(
        `  ✅ Success - Processing time: ${result.metadata.processingTime}ms`
      );
      console.log(
        `  📊 Result: ${JSON.stringify(result.data, null, 2).substring(0, 100)}...`
      );
    } else {
...
async function demonstratePlatformMonitoring(): Promise<void> {
  ...
  for (const [serviceId, health] of Object.entries(status.services)) {
    if (health.status === 'healthy') {
      ...
      unhealthyCount++;
      console.log(
        `  ⚠️  ${serviceId}: ${health.status} - ${health.issues.join(', ')}`
      );
    }
  }
  ...
  const orchestrationSteps = [
    {
      ...
      description: 'Ingest threat data',
    },
    {
      service: 'ioc-validation',
      operation: 'validateIOC',
      description: 'Validate IOC quality',
    },
    {
    ...
    {
      service: 'ioc-enrichment',
      operation: 'enrichIOC',
      description: 'Enrich with intelligence',
    },
    {
...
async function demonstrateAPIDocumentation(): Promise<void> {
  ...
  console.log(
    `📋 Generated unified API documentation with ${Object.keys(docs.paths).length} endpoints`
  );
