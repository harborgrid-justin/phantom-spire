...
interface ThreatActorOverviewProps {
  status: ThreatActorStatus | undefined;
}

export const ThreatActorOverview: React.FC<ThreatActorOverviewProps> = ({ status }) => {
  if (!status?.data) {
    return (
  ...
  if (!metrics) {
    return (
      <Alert severity="info">Threat Actor metrics are currently being initialized...</Alert>
    );
  }
        ...
        <Card>
          <CardContent>
            ...
            <Box display="flex" alignItems="center">
              <CircularProgress
                variant="determinate"
                value={metrics.attribution_confidence || 0}
                size={60}
              ...
              <Box ml={2}>
                <Typography variant="h4" color={getConfidenceColor(metrics.attribution_confidence || 0)}>
                  {(metrics.attribution_confidence || 0).toFixed(0)}%
                </Typography>
