...
#[napi]
impl PhantomXdrCore {
    /// Create a new XDR Core instance with configuration
    #[napi(constructor)]
    pub fn new(config_json: Option<String>) -> Result<Self> {
        // Parse configuration from JSON if provided
        let _config = if let Some(json_str) = config_json {
            serde_json::from_str::<serde_json::Value>(&json_str)
                .map_err(|e| napi::Error::from_reason(format!("Invalid JSON configuration: {}", e)))?
        } else {
