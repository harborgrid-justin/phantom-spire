...
impl ImpactAssessmentModule {
    ...
    pub async fn assess_impact(&self, incident: &ThreatIncident) -> Result<ImpactAssessment> {
        ...
        let business_impact = self
            ...
            .analyze_business_impact(incident)
            .await?;
        ...
        let financial_impact = self
            ...
            .quantify_financial_impact(incident)
            .await?;
        ...
        let overall_score = self.calculate_overall_impact_score(
            &business_impact,
            &technical_impact,
            &operational_impact,
            &financial_impact,
        ...
        let recovery_plan = self
            .recovery_planner
            .generate_recovery_plan(incident, &overall_score)
            .await?;
        ...
        self.assessment_cache
            .write()
            .await
            .insert(incident.incident_id.clone(), cached);

        // Send assessment event
        self.send_assessment_event(AssessmentEvent::AssessmentCompleted(assessment.clone()))
            .await?;
    ...
    async fn assess_technical_impact(&self, incident: &ThreatIncident) -> Result<TechnicalImpact> {
        ...
        Ok(TechnicalImpact {
            ...
            critical_systems_affected: affected_systems
                .iter()
                .any(|s| s.criticality == SystemCriticality::Critical),
            data_sensitivity_level: self.determine_data_sensitivity(&data_compromised),
        })
    }

    /// Assess operational impact
    async fn assess_operational_impact(
        &self,
        ...
        let productivity_loss = self
            .calculate_productivity_loss(&business_processes_affected, operational_downtime)
            .await?;

        let operational_score = self.calculate_operational_score(
        ...
        Ok(OperationalImpact {
            ...
            mission_critical_processes_affected: business_processes_affected
                .iter()
                .any(|p| p.criticality == ProcessCriticality::MissionCritical),
        })
    }

    /// Assess reputational impact
    async fn assess_reputational_impact(
        &self,
        incident: &ThreatIncident,
    ) -> Result<ReputationalImpact> {
        ...
        let brand_damage = self
            .calculate_brand_damage(&media_coverage, &customer_trust_impact)
            .await?;

        let reputational_score = self.calculate_reputational_score(
            &media_coverage,
        ...
        Ok(ReputationalImpact {
            ...
            long_term_reputation_risk: self
                .assess_long_term_risk(&media_coverage, &customer_trust_impact),
        })
    }
    ...
    async fn assess_compliance_impact(
        ...
        let compliance_breach_severity =
            self.assess_breach_severity(&regulatory_violations).await?;
        let reporting_requirements = self
            .determine_reporting_requirements(&regulatory_violations)
            .await?;
        let penalty_exposure = self
            .calculate_penalty_exposure(&regulatory_violations)
            .await?;

        let compliance_score =
            self.calculate_compliance_score(&regulatory_violations, &compliance_breach_severity);
        ...
        Ok(ComplianceImpact {
            ...
            regulatory_attention_required: compliance_breach_severity == BreachSeverity::High
                || compliance_breach_severity == BreachSeverity::Critical,
        })
    }
    ...
    fn calculate_overall_impact_score(
        ...
        let weighted_score = (business.business_impact_score * weights.business
    ...
    fn calculate_assessment_confidence(&self, incident: &ThreatIncident) -> f64 {
        ...
        let data_completeness = if incident.description.is_some() {
            0.9
        } else {
            0.6
        };
        let evidence_quality = if !incident.evidence.is_empty() {
            0.85
        } else {
            0.5
        };
        let impact_indicators = if incident.impact_indicators.is_some() {
            0.8
        } else {
            0.4
        };
    ...
    async fn generate_impact_recommendations(
        &self,
    ...
    async fn identify_affected_processes(
        &self,
        incident: &ThreatIncident,
    ) -> Result<Vec<AffectedProcess>> {
        // Identify business processes affected by the incident
        Ok(vec![AffectedProcess {
    ...
    async fn calculate_productivity_loss(
        &self,
        processes: &[AffectedProcess],
        downtime: f64,
    ) -> Result<f64> {
        let total_process_importance: f64 = processes
            .iter()
    ...
    async fn assess_customer_trust_impact(
        &self,
        _incident: &ThreatIncident,
    ) -> CustomerTrustImpact {
        CustomerTrustImpact {
            trust_impact_score: 6.5,
            affected_customer_segments: vec![
                "Retail Customers".to_string(),
                "Enterprise Clients".to_string(),
            ],
    ...
    async fn assess_stakeholder_confidence(
        &self,
        _incident: &ThreatIncident,
    ) -> StakeholderConfidenceImpact {
        StakeholderConfidenceImpact {
            confidence_impact_score: 7.0,
            affected_stakeholders: vec![
                "Investors".to_string(),
                "Board Members".to_string(),
                "Partners".to_string(),
            ],
    ...
    async fn calculate_brand_damage(
        &self,
        media: &MediaCoverage,
    ...
    fn assess_long_term_risk(
        ...
        if media.level == MediaCoverageLevel::High && customer.churn_risk_percentage > 20.0 {
            LongTermRisk::High
        } else if media.level == MediaCoverageLevel::Moderate
            || customer.churn_risk_percentage > 10.0
        {
            LongTermRisk::Medium
        } else {
            LongTermRisk::Low
    ...
    async fn identify_regulatory_violations(
        &self,
        incident: &ThreatIncident,
    ) -> Result<Vec<RegulatoryViolation>> {
        // Identify potential regulatory violations based on incident characteristics
        Ok(vec![RegulatoryViolation {
    ...
    async fn assess_breach_severity(
        &self,
        violations: &[RegulatoryViolation],
    ) -> Result<BreachSeverity> {
        let max_penalty = violations
            .iter()
    ...
    async fn determine_reporting_requirements(
        &self,
        violations: &[RegulatoryViolation],
    ) -> Result<Vec<ReportingRequirement>> {
    ...
    fn determine_data_sensitivity(&self, compromise: &DataCompromise) -> DataSensitivity {
        if compromise
            ...
            .contains(&DataType::HealthInformation)
            || compromise
                .data_types_compromised
                .contains(&DataType::FinancialData)
        {
            DataSensitivity::Critical
        } else if compromise
            .data_types_compromised
            .contains(&DataType::PersonalInformation)
        {
...
impl QuantificationEngine {
    ...
    async fn quantify_financial_impact(
        &self,
        _incident: &ThreatIncident,
        ...
        let total_cost =
            direct_costs + indirect_costs + opportunity_costs + regulatory_fines + insurance_claims;
...
impl BusinessImpactAnalyzer {
    ...
    async fn analyze_business_impact(&self, incident: &ThreatIncident) -> Result<BusinessImpact> {
        ...
        let business_score = ((revenue_impact.abs() / 1000000.0)
            ...
            + (competitive_advantage_impact.abs() * 10.0))
            / 3.0;
            ...
            ImpactLevel::Critical => vec![
                RecoveryPhase {
                    ...
                    success_criteria: vec![
                        "Threat contained".to_string(),
                        "Communication established".to_string(),
                    ],
                },
                RecoveryPhase {
                ...
                RecoveryPhase {
                    ...
                    required_resources: vec![
                        ...
                        "Support Teams".to_string(),
                    ],
                    success_criteria: vec![
                        "Operations resumed".to_string(),
                        "Customer communication completed".to_string(),
                    ],
                },
            ],
            _ => vec![
                RecoveryPhase {
                    ...
                    success_criteria: vec![
                        "Impact assessed".to_string(),
                        "Recovery plan developed".to_string(),
                    ],
                },
                RecoveryPhase {
                    ...
                    success_criteria: vec![
                        "Systems recovered".to_string(),
                        "Operations restored".to_string(),
                    ],
                },
            ],
        };
