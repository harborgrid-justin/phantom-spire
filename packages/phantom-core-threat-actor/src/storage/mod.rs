        ...
        match backend {
            ...
            StorageBackend::PostgreSQL => {
                #[cfg(feature = "postgres-store")]
                {
                    let connection_string = connection_string.ok_or_else(|| {
                        StorageError::Configuration(
                            "PostgreSQL connection string required".to_string(),
                        )
                    })?;
                ...
                #[cfg(not(feature = "postgres-store"))]
                {
                    Err(StorageError::Configuration(
                        "PostgreSQL storage not enabled. Enable with 'postgres-store' feature"
                            .to_string(),
                    ))
                }
            }
            StorageBackend::MongoDB => {
                #[cfg(feature = "mongodb-store")]
                {
                    let connection_string = connection_string.ok_or_else(|| {
                        StorageError::Configuration(
                            "MongoDB connection string required".to_string(),
                        )
                    })?;
                    ...
                    let storage =
                        MongoDBStorage::new(&connection_string, "phantom_threat_actors").await?;
                    Ok(Arc::new(storage))
                }
                #[cfg(not(feature = "mongodb-store"))]
                {
                    Err(StorageError::Configuration(
                        "MongoDB storage not enabled. Enable with 'mongodb-store' feature"
                            .to_string(),
                    ))
                }
            }
            StorageBackend::Redis => {
                ...
                {
                    Err(StorageError::Configuration(
                        "Redis storage not enabled. Enable with 'redis-store' feature".to_string(),
                    ))
                }
            }
            StorageBackend::Elasticsearch => {
                #[cfg(feature = "elasticsearch-store")]
                {
                    let connection_string = connection_string.ok_or_else(|| {
                        StorageError::Configuration(
                            "Elasticsearch connection string required".to_string(),
                        )
                    })?;
                    ...
                    let storage =
                        ElasticsearchStorage::new(&connection_string, "phantom_threat_actors")
                            .await?;
                    Ok(Arc::new(storage))
                }
