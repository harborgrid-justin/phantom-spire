...
use chrono::Utc;
use elasticsearch::{
    BulkOperation, BulkOperations, BulkParts, DeleteParts, Elasticsearch, IndexParts, SearchParts,
};
...
impl ElasticsearchStorage {
    ...
    pub async fn new(connection_string: &str, index_name: &str) -> Result<Self, StorageError> {
        let transport = elasticsearch::http::transport::Transport::single_node(connection_string)
            .map_err(|e| {
            StorageError::Connection(format!("Failed to create transport: {}", e))
        })?;

        let client = Elasticsearch::new(transport);

        // Test connection
        let response = client.info().send().await.map_err(|e| {
            StorageError::Connection(format!("Failed to connect to Elasticsearch: {}", e))
        })?;

        if !response.status_code().is_success() {
            return Err(StorageError::Connection(
                "Elasticsearch connection failed".to_string(),
            ));
        }
        ...
        match response {
            Ok(resp) => {
                if !resp.status_code().is_success()
                    && resp.status_code() != elasticsearch::http::StatusCode::BAD_REQUEST
                {
    ...
    fn document_to_actor(doc: &Value) -> Result<ThreatActor, StorageError> {
        let source = doc.get("_source").ok_or_else(|| {
            StorageError::Serialization("Missing _source in document".to_string())
        })?;
        ...
        Ok(ThreatActor {
            ...
            .map_err(|e| {
                StorageError::Serialization(format!("Failed to parse first_observed: {}", e))
            })?
            .with_timezone(&Utc),
            ...
            .map_err(|e| {
                StorageError::Serialization(format!("Failed to parse last_activity: {}", e))
            })?
            .with_timezone(&Utc),
            ...
            capabilities: serde_json::from_value(
                source.get("capabilities").cloned().unwrap_or(json!([])),
            )
            .map_err(|e| {
                StorageError::Serialization(format!("Failed to deserialize capabilities: {}", e))
            })?,
            infrastructure: serde_json::from_value(
                source.get("infrastructure").cloned().unwrap_or(json!({})),
            )
            .map_err(|e| {
            ...
            campaigns: source
                ...
                .map(|s| s.to_string())
                .collect(),
            associated_malware: source
                .get("associated_malware")
                .and_then(|v| v.as_array())
                .unwrap_or(&vec![])
                .iter()
                .filter_map(|v| v.as_str())
            ...
            iocs: source
                ...
                .map(|s| s.to_string())
                .collect(),
            relationships: serde_json::from_value(
                source.get("relationships").cloned().unwrap_or(json!([])),
            )
            .map_err(|e| {
    ...
    fn document_to_campaign(doc: &Value) -> Result<Campaign, StorageError> {
        ...
        Ok(Campaign {
            ...
            .map_err(|e| StorageError::Serialization(format!("Failed to parse start_date: {}", e)))?
            .with_timezone(&Utc),
            end_date: source
                .get("end_date")
                .and_then(|v| v.as_str())
            ...
            iocs: source
                ...
                .map(|s| s.to_string())
                .collect(),
            impact_assessment: serde_json::from_value(
                source
...
impl ThreatActorStorage for ElasticsearchStorage {
    ...
    async fn health_check(&self) -> Result<HealthStatus, StorageError> {
        ...
        if !response.status_code().is_success() {
            return Err(StorageError::Internal(
                "Elasticsearch health check failed".to_string(),
            ));
        }

        let response_body: Value = response.json().await.map_err(|e| {
            StorageError::Internal(format!("Failed to parse health check response: {}", e))
        })?;
    ...
    async fn store_threat_actor_batch(&self, actors: &[ThreatActor]) -> Result<(), StorageError> {
        ...
        for actor in actors {
            let doc = Self::actor_to_document(actor)?;
            operations
                .push(BulkOperation::index(doc).id(&actor.id))
                .map_err(|e| {
                    StorageError::Internal(format!("Failed to create bulk operation: {}", e))
                })?;
        }
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                StorageError::Internal(format!("Failed to bulk store threat actors: {}", e))
            })?;
    ...
    async fn get_threat_actor_batch(
        ...
        let response_body: Value = response.json().await.map_err(|e| {
            StorageError::Internal(format!("Failed to parse batch response: {}", e))
        })?;
    ...
    async fn search_threat_actors(
        ...
        if let Some(ref actor_types) = criteria.actor_types {
            let types: Vec<String> = actor_types
                .iter()
                .map(|t| serde_json::to_string(t).unwrap_or_else(|_| "Unknown".to_string()))
                .collect();
        ...
        if let Some(ref sophistication_levels) = criteria.sophistication_levels {
            let levels: Vec<String> = sophistication_levels
                .iter()
                .map(|l| serde_json::to_string(l).unwrap_or_else(|_| "Unknown".to_string()))
                .collect();
        ...
        if let Some(after) = criteria.first_observed_after {
            query_parts
                .push(json!({ "range": { "first_observed": { "gte": after.to_rfc3339() } } }));
        }

        if let Some(before) = criteria.first_observed_before {
            query_parts
                .push(json!({ "range": { "first_observed": { "lte": before.to_rfc3339() } } }));
        }

        if let Some(after) = criteria.last_activity_after {
            query_parts
                .push(json!({ "range": { "last_activity": { "gte": after.to_rfc3339() } } }));
        }

        if let Some(before) = criteria.last_activity_before {
            query_parts
                .push(json!({ "range": { "last_activity": { "lte": before.to_rfc3339() } } }));
        }
        ...
        let response_body: Value = response.json().await.map_err(|e| {
            StorageError::Internal(format!("Failed to parse search response: {}", e))
        })?;
    ...
    async fn search_campaigns(
        ...
        if !response.status_code().is_success() {
            return Err(StorageError::Internal(format!(
                "Search campaigns failed: {}",
                response.status_code()
            )));
        }

        let response_body: Value = response.json().await.map_err(|e| {
            StorageError::Internal(format!("Failed to parse search campaigns response: {}", e))
        })?;
    ...
    async fn store_attribution_analysis(
        ...
        if !response.status_code().is_success() {
            return Err(StorageError::Internal(format!(
                "Failed to index attribution analysis: {}",
                response.status_code()
            )));
        }
    ...
    async fn get_attribution_analysis(
        ...
        if !response.status_code().is_success() {
            return Err(StorageError::Internal(format!(
                "Get attribution analysis failed: {}",
                response.status_code()
            )));
        }
        ...
        let response_body: Value = response.json().await.map_err(|e| {
            StorageError::Internal(format!(
                "Failed to parse attribution analysis response: {}",
                e
            ))
        })?;
        ...
        for hit in hits {
            if let Some(source) = hit.get("_source") {
                analyses.push(AttributionAnalysis {
                    primary_attribution: source
                        .get("primary_attribution")
                        .and_then(|v| v.as_str())
                        .map(|s| s.to_string()),
                    alternative_attributions: serde_json::from_value(
                        source
                            .get("alternative_attributions")
                            .cloned()
                            .unwrap_or(json!([])),
                    )
                    .map_err(|e| {
                    ...
                    confidence_score: source
                        ...
                        .and_then(|v| v.as_f64())
                        .unwrap_or(0.0),
                    evidence_summary: serde_json::from_value(
                        source.get("evidence_summary").cloned().unwrap_or(json!([])),
                    )
                    .map_err(|e| {
                        StorageError::Serialization(format!(
                    ...
                    .map_err(|e| {
                        StorageError::Serialization(format!(
                            "Failed to parse analysis_timestamp: {}",
                            e
                        ))
                    })?
                    .with_timezone(&Utc),
    ...
    async fn store_behavioral_analysis(
        ...
        if !response.status_code().is_success() {
            return Err(StorageError::Internal(format!(
                "Failed to index behavioral analysis: {}",
                response.status_code()
            )));
        }
    ...
    async fn get_behavioral_analysis(
        ...
        if !response.status_code().is_success() {
            return Err(StorageError::Internal(format!(
                "Get behavioral analysis failed: {}",
                response.status_code()
            )));
        }
        ...
        let response_body: Value = response.json().await.map_err(|e| {
            StorageError::Internal(format!(
                "Failed to parse behavioral analysis response: {}",
                e
            ))
        })?;
    ...
    async fn update_actor_confidence(
        ...
        if !response.status_code().is_success() {
            return Err(StorageError::Internal(format!(
                "Failed to update actor confidence: {}",
                response.status_code()
            )));
        }
    ...
    async fn get_actors_by_type(
        ...
        if !response.status_code().is_success() {
            return Err(StorageError::Internal(format!(
                "Get actors by type failed: {}",
                response.status_code()
            )));
        }

        let response_body: Value = response.json().await.map_err(|e| {
            StorageError::Internal(format!("Failed to parse actors by type response: {}", e))
        })?;
    ...
    async fn get_actors_by_sophistication(
        ...
        if !response.status_code().is_success() {
            return Err(StorageError::Internal(format!(
                "Get actors by sophistication failed: {}",
                response.status_code()
            )));
        }
        ...
        let response_body: Value = response.json().await.map_err(|e| {
            StorageError::Internal(format!(
                "Failed to parse actors by sophistication response: {}",
                e
            ))
        })?;
    ...
    async fn get_actors_by_country(&self, country: &str) -> Result<Vec<ThreatActor>, StorageError> {
        ...
        if !response.status_code().is_success() {
            return Err(StorageError::Internal(format!(
                "Get actors by country failed: {}",
                response.status_code()
            )));
        }

        let response_body: Value = response.json().await.map_err(|e| {
            StorageError::Internal(format!("Failed to parse actors by country response: {}", e))
        })?;
    ...
    async fn get_statistics(&self) -> Result<StorageStatistics, StorageError> {
        ...
        for (doc_type, query) in queries {
            let response = self
                ...
                .await
                .map_err(|e| {
                    StorageError::Internal(format!("Failed to get {} count: {}", doc_type, e))
                })?;

            if response.status_code().is_success() {
                let response_body: Value = response.json().await.map_err(|e| {
                    StorageError::Internal(format!(
                        "Failed to parse {} count response: {}",
                        doc_type, e
                    ))
                })?;
