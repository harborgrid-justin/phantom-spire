...
use redis::{Client, Commands, AsyncCommands, Connection, aio::MultiplexedConnection};
...
impl RedisStorage {
    ...
    async fn add_to_index(&mut self, index_key: &str, value: &str, member: &str) -> Result<(), StorageError> {
        self.connection.sadd(index_key, member).await
            .map_err(|e| StorageError::Internal(format!("Failed to add to index {}: {}", index_key, e)))?;
        Ok(())
    }

    /// Remove actor from index
    async fn remove_from_index(&mut self, index_key: &str, member: &str) -> Result<(), StorageError> {
        self.connection.srem(index_key, member).await
            .map_err(|e| StorageError::Internal(format!("Failed to remove from index {}: {}", index_key, e)))?;
        Ok(())
    }

    /// Get members from index
    async fn get_from_index(&mut self, index_key: &str) -> Result<Vec<String>, StorageError> {
        let members: Vec<String> = self.connection.smembers(index_key).await
            .map_err(|e| StorageError::Internal(format!("Failed to get from index {}: {}", index_key, e)))?;
        Ok(members)
    }
}
...
impl ThreatActorStorage for RedisStorage {
    ...
    async fn store_threat_actor(&self, actor: &ThreatActor) -> Result<(), StorageError> {
        ...
        let data = serde_json::to_string(actor)
            .map_err(|e| StorageError::Serialization(format!("Failed to serialize threat actor: {}", e)))?;
        ...
        // Update indexes
        let mut async_conn = self.client.get_multiplexed_async_connection().await
            .map_err(|e| StorageError::Connection(format!("Failed to get async connection: {}", e)))?;

        // Index by type
        let type_index = Self::index_key(&format!("actor_type:{}", serde_json::to_string(&actor.actor_type).unwrap_or_else(|_| "Unknown".to_string())));
        async_conn.sadd(&type_index, &actor.id).await
        ...
        if let Some(ref country) = actor.origin_country {
            ...
            async_conn.sadd(&country_index, &actor.id).await
                .map_err(|e| StorageError::Internal(format!("Failed to index actor by country: {}", e)))?;
        }

        // Index by status
        let status_index = Self::index_key(&format!("status:{}", serde_json::to_string(&actor.status).unwrap_or_else(|_| "Unknown".to_string())));
        async_conn.sadd(&status_index, &actor.id).await
            .map_err(|e| StorageError::Internal(format!("Failed to index actor by status: {}", e)))?;

        // Add to all actors index
        let all_index = Self::index_key("all_actors");
        async_conn.sadd(&all_index, &actor.id).await
            .map_err(|e| StorageError::Internal(format!("Failed to add to all actors index: {}", e)))?;
    ...
    async fn get_threat_actor(&self, id: &str) -> Result<Option<ThreatActor>, StorageError> {
        ...
        match data {
            Some(json) => {
                let actor: ThreatActor = serde_json::from_str(&json)
                    .map_err(|e| StorageError::Serialization(format!("Failed to deserialize threat actor: {}", e)))?;
                Ok(Some(actor))
            },
            None => Ok(None),
        }
    ...
    async fn get_threat_actor_batch(&self, ids: &[String]) -> Result<Vec<ThreatActor>, StorageError> {
        ...
        for id in ids {
            let key = Self::actor_key(id);
            if let Some(data) = conn.get::<_, Option<String>>(&key)
                .map_err(|e| StorageError::Internal(format!("Failed to get threat actor {}: {}", id, e)))? {
                let actor: ThreatActor = serde_json::from_str(&data)
                    .map_err(|e| StorageError::Serialization(format!("Failed to deserialize threat actor {}: {}", id, e)))?;
                actors.push(actor);
            }
        }
    ...
    async fn search_threat_actors(&self, criteria: &ThreatActorSearchCriteria) -> Result<Vec<ThreatActor>, StorageError> {
        ...
        if criteria.actor_types.is_none() && criteria.origin_countries.is_none() && criteria.activity_status.is_none() {
            let all_index = Self::index_key("all_actors");
            candidate_ids = async_conn.smembers(&all_index).await
            ...
            if let Some(ref actor_types) = criteria.actor_types {
                for actor_type in actor_types {
                    ...
                    let ids: Vec<String> = async_conn.smembers(&type_index).await
            ...
            if let Some(ref countries) = criteria.origin_countries {
                for country in countries {
                    let country_index = Self::index_key(&format!("country:{}", country));
                    let ids: Vec<String> = async_conn.smembers(&country_index).await
            ...
            if let Some(ref statuses) = criteria.activity_status {
                for status in statuses {
                    let status_str = serde_json::to_string(status).unwrap_or_else(|_| "Unknown".to_string());
                    let status_index = Self::index_key(&format!("status:{}", status_str));
                    let ids: Vec<String> = async_conn.smembers(&status_index).await
                        .map_err(|e| StorageError::Internal(format!("Failed to get actors by status: {}", e)))?;
                    candidate_sets.push(ids);
                }
            }
    ...
    async fn list_threat_actor_ids(&self) -> Result<Vec<String>, StorageError> {
        ...
        let all_index = Self::index_key("all_actors");
        let ids: Vec<String> = async_conn.smembers(&all_index).await
    ...
    async fn delete_threat_actor(&self, id: &str) -> Result<bool, StorageError> {
        ...
        let exists: bool = conn.exists(&key)
            .map_err(|e| StorageError::Internal(format!("Failed to check if threat actor exists: {}", e)))?;

        if exists {
            // Get actor data to clean up indexes
            let data: String = conn.get(&key)
                .map_err(|e| StorageError::Internal(format!("Failed to get threat actor for deletion: {}", e)))?;

            let actor: ThreatActor = serde_json::from_str(&data)
                .map_err(|e| StorageError::Serialization(format!("Failed to deserialize threat actor for deletion: {}", e)))?;
            ...
            let type_index = Self::index_key(&format!("actor_type:{}", serde_json::to_string(&actor.actor_type).unwrap_or_else(|_| "Unknown".to_string())));
            async_conn.srem(&type_index, id).await
                .map_err(|e| StorageError::Internal(format!("Failed to remove from type index: {}", e)))?;

            if let Some(ref country) = actor.origin_country {
                let country_index = Self::index_key(&format!("country:{}", country));
                async_conn.srem(&country_index, id).await
                    .map_err(|e| StorageError::Internal(format!("Failed to remove from country index: {}", e)))?;
            }

            let status_index = Self::index_key(&format!("status:{}", serde_json::to_string(&actor.status).unwrap_or_else(|_| "Unknown".to_string())));
            async_conn.srem(&status_index, id).await
                .map_err(|e| StorageError::Internal(format!("Failed to remove from status index: {}", e)))?;

            let all_index = Self::index_key("all_actors");
            async_conn.srem(&all_index, id).await
                .map_err(|e| StorageError::Internal(format!("Failed to remove from all actors index: {}", e)))?;
    ...
    async fn store_campaign(&self, campaign: &Campaign) -> Result<(), StorageError> {
        ...
        let data = serde_json::to_string(campaign)
            .map_err(|e| StorageError::Serialization(format!("Failed to serialize campaign: {}", e)))?;
        ...
        async_conn.sadd(&actor_index, &campaign.id).await
            .map_err(|e| StorageError::Internal(format!("Failed to index campaign by actor: {}", e)))?;

        // Index by status
        let status_index = Self::index_key(&format!("campaigns_by_status:{}", serde_json::to_string(&campaign.status).unwrap_or_else(|_| "Unknown".to_string())));
        async_conn.sadd(&status_index, &campaign.id).await
            .map_err(|e| StorageError::Internal(format!("Failed to index campaign by status: {}", e)))?;
        ...
        match data {
            Some(json) => {
                let campaign: Campaign = serde_json::from_str(&json)
                    .map_err(|e| StorageError::Serialization(format!("Failed to deserialize campaign: {}", e)))?;
                Ok(Some(campaign))
            },
            None => Ok(None),
        }
    ...
    async fn search_campaigns(&self, actor_id: Option<&str>, status: Option<CampaignStatus>) -> Result<Vec<Campaign>, StorageError> {
        ...
        if let Some(actor_id) = actor_id {
            ...
            campaign_ids = async_conn.smembers(&actor_index).await
                .map_err(|e| StorageError::Internal(format!("Failed to get campaigns by actor: {}", e)))?;
        } else if let Some(ref status) = status {
            let status_str = serde_json::to_string(status).unwrap_or_else(|_| "Unknown".to_string());
            let status_index = Self::index_key(&format!("campaigns_by_status:{}", status_str));
            campaign_ids = async_conn.smembers(&status_index).await
                .map_err(|e| StorageError::Internal(format!("Failed to get campaigns by status: {}", e)))?;
        } else {
        ...
        for id in campaign_ids {
            let key = Self::campaign_key(&id);
            if let Some(data) = conn.get::<_, Option<String>>(&key)
                .map_err(|e| StorageError::Internal(format!("Failed to get campaign {}: {}", id, e)))? {
                let campaign: Campaign = serde_json::from_str(&data)
                    .map_err(|e| StorageError::Serialization(format!("Failed to deserialize campaign {}: {}", id, e)))?;
    ...
    async fn store_attribution_analysis(&self, analysis: &AttributionAnalysis) -> Result<(), StorageError> {
        ...
        let data = serde_json::to_string(analysis)
            .map_err(|e| StorageError::Serialization(format!("Failed to serialize attribution analysis: {}", e)))?;
    ...
    async fn store_behavioral_analysis(&self, analysis: &BehavioralAnalysis) -> Result<(), StorageError> {
        ...
        let data = serde_json::to_string(analysis)
            .map_err(|e| StorageError::Serialization(format!("Failed to serialize behavioral analysis: {}", e)))?;
    ...
    async fn get_behavioral_analysis(&self, actor_id: &str) -> Result<Option<BehavioralAnalysis>, StorageError> {
        ...
        match data {
            Some(json) => {
                let analysis: BehavioralAnalysis = serde_json::from_str(&json)
                    .map_err(|e| StorageError::Serialization(format!("Failed to deserialize behavioral analysis: {}", e)))?;
                Ok(Some(analysis))
            },
            None => Ok(None),
        }
    ...
    async fn update_actor_confidence(&self, actor_id: &str, confidence: f64) -> Result<(), StorageError> {
        ...
        let data: Option<String> = conn.get(&key)
            .map_err(|e| StorageError::Internal(format!("Failed to get threat actor for update: {}", e)))?;

        if let Some(json) = data {
            let mut actor: ThreatActor = serde_json::from_str(&json)
                .map_err(|e| StorageError::Serialization(format!("Failed to deserialize threat actor for update: {}", e)))?;

            actor.confidence_score = confidence;
            let updated_data = serde_json::to_string(&actor)
                .map_err(|e| StorageError::Serialization(format!("Failed to serialize updated threat actor: {}", e)))?;

            conn.set(&key, &updated_data)
                .map_err(|e| StorageError::Internal(format!("Failed to update threat actor confidence: {}", e)))?;
        }
    ...
    async fn get_actors_by_type(&self, actor_type: ActorType) -> Result<Vec<ThreatActor>, StorageError> {
        ...
        let ids: Vec<String> = async_conn.smembers(&index_key).await
            .map_err(|e| StorageError::Internal(format!("Failed to get actors by type: {}", e)))?;
    ...
    async fn get_statistics(&self) -> Result<StorageStatistics, StorageError> {
        ...
        let all_index = Self::index_key("all_actors");
        let threat_actor_count = async_conn.scard(&all_index).await
