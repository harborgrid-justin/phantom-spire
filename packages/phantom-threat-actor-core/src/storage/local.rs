...
impl ThreatActorStorage for LocalStorage {
    ...
    async fn health_check(&self) -> Result<HealthStatus, StorageError> {
        ...
        let _actors = self.threat_actors.read().map_err(|e| {
    ...
    async fn store_threat_actor(&self, actor: &ThreatActor) -> Result<(), StorageError> {
        let mut actors = self.threat_actors.write().map_err(|e| {
    ...
    async fn store_threat_actor_batch(&self, actors: &[ThreatActor]) -> Result<(), StorageError> {
        let mut storage = self.threat_actors.write().map_err(|e| {
            StorageError::Internal(format!("Failed to acquire write lock: {}", e))
        })?;
    ...
    async fn get_threat_actor(&self, id: &str) -> Result<Option<ThreatActor>, StorageError> {
        let actors = self.threat_actors.read().map_err(|e| {
            StorageError::Internal(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn store_attribution_analysis(&self, analysis: &AttributionAnalysis) -> Result<(), StorageError> {
        ...
        let actor_id = analysis.primary_attribution.as_ref().unwrap_or(&"unknown".to_string()).clone();
        
        let mut analyses = self.attribution_analyses.write().map_err(|e| {
            StorageError::Internal(format!("Failed to acquire write lock: {}", e))
        })?;
        
        analyses.entry(actor_id).or_insert_with(Vec::new).push(analysis.clone());
        Ok(())
    }

    async fn get_attribution_analysis(&self, actor_id: &str) -> Result<Vec<AttributionAnalysis>, StorageError> {
        let analyses = self.attribution_analyses.read().map_err(|e| {
            StorageError::Internal(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn store_behavioral_analysis(&self, analysis: &BehavioralAnalysis) -> Result<(), StorageError> {
        let mut analyses = self.behavioral_analyses.write().map_err(|e| {
            StorageError::Internal(format!("Failed to acquire write lock: {}", e))
        })?;
    ...
    async fn get_behavioral_analysis(&self, actor_id: &str) -> Result<Option<BehavioralAnalysis>, StorageError> {
        let analyses = self.behavioral_analyses.read().map_err(|e| {
            StorageError::Internal(format!("Failed to acquire read lock: {}", e))
        })?;
    ...
    async fn get_actors_by_type(&self, actor_type: ActorType) -> Result<Vec<ThreatActor>, StorageError> {
        ...
        let results: Vec<ThreatActor> = actors
            .values()
            .filter(|actor| std::mem::discriminant(&actor.actor_type) == std::mem::discriminant(&actor_type))
            .cloned()
            .collect();
    ...
    async fn get_actors_by_sophistication(&self, level: SophisticationLevel) -> Result<Vec<ThreatActor>, StorageError> {
        ...
        let results: Vec<ThreatActor> = actors
            .values()
            .filter(|actor| std::mem::discriminant(&actor.sophistication_level) == std::mem::discriminant(&level))
            .cloned()
            .collect();
    ...
    async fn get_actors_by_country(&self, country: &str) -> Result<Vec<ThreatActor>, StorageError> {
        ...
        let results: Vec<ThreatActor> = actors
            .values()
            .filter(|actor| {
                actor.origin_country.as_ref().map_or(false, |c| c == country)
            })
            .cloned()
            .collect();
    ...
    async fn get_statistics(&self) -> Result<StorageStatistics, StorageError> {
        ...
        let attribution_analyses = self.attribution_analyses.read().map_err(|e| {
            StorageError::Internal(format!("Failed to acquire read lock: {}", e))
        })?;
        let behavioral_analyses = self.behavioral_analyses.read().map_err(|e| {
            StorageError::Internal(format!("Failed to acquire read lock: {}", e))
        })?;
        
        let attribution_count = attribution_analyses.values().map(|v| v.len()).sum::<usize>() as u64;
...
impl LocalStorage {
    ...
    fn matches_criteria(&self, actor: &ThreatActor, criteria: &ThreatActorSearchCriteria) -> bool {
        if let Some(ref actor_types) = criteria.actor_types {
            if !actor_types.iter().any(|t| std::mem::discriminant(&actor.actor_type) == std::mem::discriminant(t)) {
                return false;
            }
        }
        
        if let Some(ref sophistication_levels) = criteria.sophistication_levels {
            if !sophistication_levels.iter().any(|l| std::mem::discriminant(&actor.sophistication_level) == std::mem::discriminant(l)) {
                return false;
            }
        }
        
        if let Some(ref motivations) = criteria.motivations {
            if !actor.motivation.iter().any(|m| motivations.iter().any(|cm| std::mem::discriminant(m) == std::mem::discriminant(cm))) {
                return false;
            }
        }
        ...
        if let Some(ref activity_status) = criteria.activity_status {
            if !activity_status.iter().any(|s| std::mem::discriminant(&actor.status) == std::mem::discriminant(s)) {
                return false;
            }
        }
        ...
        if let Some(ref target_sectors) = criteria.target_sectors {
            if !actor.targets.iter().any(|target| target_sectors.contains(&target.sector)) {
                return false;
            }
        }
