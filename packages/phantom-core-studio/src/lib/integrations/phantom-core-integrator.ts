...
const initializePhantomCoresInternal = async () => {
  try {
    ...
    try {
      const mlModule = await import('@phantom-core/ml-core');
    ...
    try {
      const xdrModule = await import('@phantom-core/xdr-core');
    ...
    try {
      const complianceModule = await import('@phantom-core/compliance-core');
...
export class PhantomCoreIntegrator {
  ...
  async initialize(): Promise<UnifiedPhantomResponse> {
    try {
      ...
      try {
        const { PhantomMLCore } = await import('@phantom-core/ml-core');
      ...
      try {
        const { PhantomXdrCore } = await import('@phantom-core/xdr-core/enterprise-wrapper');
      ...
      try {
        const { PhantomComplianceCore } = await import('@phantom-core/compliance-core');
  ...
  private async initializeAdditionalCores(): Promise<void> {
    ...
    const availableCores = [
      { key: 'attribution', module: '@phantom-core/attribution-core', name: 'Threat Attribution Core' },
      { key: 'crypto', module: '@phantom-core/crypto-core', name: 'Cryptographic Analysis Core' },
      { key: 'cve', module: '@phantom-core/cve-core', name: 'CVE Processing Core' },
      { key: 'feeds', module: '@phantom-core/feeds-core', name: 'Threat Feed Core' },
      { key: 'forensics', module: '@phantom-core/forensics-core', name: 'Digital Forensics Core' },
      { key: 'hunting', module: '@phantom-core/hunting-core', name: 'Threat Hunting Core' },
      { key: 'incidentResponse', module: '@phantom-core/incident-response-core', name: 'Incident Response Core' },
      { key: 'intel', module: '@phantom-core/intel-core', name: 'Threat Intelligence Core' },
      { key: 'ioc', module: '@phantom-core/ioc-core', name: 'Indicators of Compromise Core' },
      { key: 'malware', module: '@phantom-core/malware-core', name: 'Malware Analysis Core' },
      { key: 'mitre', module: '@phantom-core/mitre-core', name: 'MITRE ATT&CK Core' },
      { key: 'reputation', module: '@phantom-core/reputation-core', name: 'Reputation Analysis Core' },
      { key: 'risk', module: '@phantom-core/risk-core', name: 'Risk Assessment Core' },
      { key: 'sandbox', module: '@phantom-core/sandbox-core', name: 'Sandbox Analysis Core' },
      { key: 'secop', module: '@phantom-core/secop-core', name: 'Security Operations Core' },
      { key: 'threatActor', module: '@phantom-core/threat-actor-core', name: 'Threat Actor Core' },
      { key: 'vulnerability', module: '@phantom-core/vulnerability-core', name: 'Vulnerability Management Core' }
    ];
